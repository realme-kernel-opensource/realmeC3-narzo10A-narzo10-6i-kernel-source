 ...
 * sia81xx misc option
 ********************************************************************/
static void clear_sia81xx_err_info(sia81xx_dev_t *sia81xx)
{
	if(NULL == sia81xx)
		return ;
	...
	struct sia81xx_dev_s *sia81xx) 
{
    ...
    /* add for compat */
    if (!is_sia_chip) {
        pr_debug("[debug][%s] %s: is_sia_chip = %d. \r\n", LOG_FLAG, __func__, is_sia_chip);
        return 0;
    }
	...
	struct sia81xx_dev_s *sia81xx)
{
	...
	if(0 == sia81xx->disable_pin) {
		...
		mdelay(1);/* wite chip power up, the time must be > 1ms */
	...
	char* buf)
{
    ...
    memset((void *)&vals[0], 0x00, sizeof(vals));

	sia81xx_regmap_read(
		sia81xx->regmap, 0x00, 1, (char *)&chip_id);
	sia81xx_regmap_read(
	...
	size_t len)
{
	...
	switch(simple_strtoul(strsep(&cur, split_symb), &after, 10)) {
		...
		case SIA81XX_CMD_GET_REG :
		{
			...
			if(0 != sia81xx_regmap_read(sia81xx->regmap, addr, 1, (char *)&val)) {
		...
		case SIA81XX_CMD_GET_CHIP_TYPE :
		{
			...
			pos = 0;
			vfs_write(fp, chip_name, strlen(chip_name), &pos);
...
static int sia81xx_probe(struct platform_device *pdev)
{
	...
	if(0 == disable_pin) {
		...
		/* get owi pin's sn */
		owi_pin = 
			of_get_named_gpio(pdev->dev.of_node, "si,sia81xx_owi", 0);
		if(owi_pin < 0)	{
...
{	
	...
	pr_info("[info][%s] %s: running, version : %s \r\n", 
