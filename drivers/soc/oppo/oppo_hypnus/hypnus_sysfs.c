...
/* sysfs */
static ssize_t show_version(struct hypnus_data *pdata, char *buf)
{
	return 0;
}

static ssize_t store_not_preferred(struct hypnus_data *pdata,
				   const char *buf, size_t count)
{
	return count;
}

static ssize_t show_not_preferred(struct hypnus_data *pdata, char *buf)
{
	return 0;
}
...
{
	struct hypnus_data *pdata = to_hypnus_data(kobj);
	struct hypnus_attr *cattr = to_attr(attr);
	ssize_t ret = -EIO;

	if (cattr->show)
		ret = cattr->show(pdata, buf);
		     ...
		     const char *buf, size_t count)
{
	...
	if (cattr->store)
		ret = cattr->store(pdata, buf, count);
...
int hypnus_sysfs_init(struct hypnus_data *pdata)
{
	int ret;

	kobject_init(&pdata->kobj, &ktype_hypnus);

	ret = kobject_add(&pdata->kobj, kernel_kobj, "hypnus");
	if (ret) {
...
void hypnus_sysfs_remove(struct hypnus_data *pdata)
{
	kobject_put(&pdata->kobj);
}
