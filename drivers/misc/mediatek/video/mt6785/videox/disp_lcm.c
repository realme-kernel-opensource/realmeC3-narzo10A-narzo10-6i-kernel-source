	...
	enum LCM_INTERFACE_ID lcm_id, int is_lcm_inited)
{
	...
	if (_lcm_count() == 0) {
		...
		if (plcm_name == NULL) {
			...
			if (!isLCMFound) {
				DISP_PR_ERR(
					"FATAL ERROR: cannot find lcm driver:%s in linux kernel driver\n",
					    plcm_name);
	...
	if (plcm && lcm_param) {
		...
		DISP_PR_ERR(
			"FATAL ERROR! kzalloc plcm and plcm->params failed\n");
		goto fail;
	}
					   ...
					   int is_lcm_inited)
{
	...
	if (_lcm_count() < 2) {
		DISP_PR_ERR(
			"no ext lcm driver defined in linux kernel driver\n");
		return NULL;
	...
	(CONFIG_MTK_DUAL_DISPLAY_SUPPORT == 2)
			...
			if (lcm_drv == NULL) {
				DISP_PR_ERR(
					"FATAL ERROR! Cannot find EXT_LCM Driver. The LK ext lcm name is(%s)\n",
					    plcm_name);
				return NULL;
			}
	...
	DISP_PR_ERR(
		"EXT LCM Interface [%d] didn't define any lcm driver\n",
		    lcm_id);
		    ...
		    int force)
{
	...
	if (lcm_drv->update) {
		...
		if (!disp_lcm_is_video_mode(plcm))
			DISP_PR_ERR(
			"lcm is cmd mode lcm_drv->update is null\n");
		return -1;
	}
...
unsigned int disp_lcm_ATA(struct disp_lcm_handle *plcm)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (lcm_drv->ata_check) {
			...
			DISP_PR_ERR(
				"FATAL ERROR, lcm_drv->ata_check is null\n");
			return 0;
		}
			 ...
			 unsigned int *lcm_value)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (lcm_drv->set_lcm_cmd) {
			...
			DISP_PR_ERR(
				"FATAL ERROR, lcm_drv->set_lcm_cmd is null\n");
			return -1;
		}
