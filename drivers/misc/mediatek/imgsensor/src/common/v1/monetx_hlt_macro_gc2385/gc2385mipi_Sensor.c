...
#define GC2385_DEFAULT_DUMMY_LINE_NUMS    0x10 /* VB */

static kal_uint32 Basic_Dummy_Line = GC2385_DEFAULT_DUMMY_LINE_NUMS;
/****************************Modify Following Strings for Debug****************************/
#define PFX "GC2385_camera_sensor"
#define LOG_1 LOG_INF("GC2385,MIPI 1LANE\n")
#define LOG_2 LOG_INF("preview 2M@30fps,864Mbps/lane;video 5M@30fps,864Mbps/lane; capture 5M@30fps,864Mbps/lane\n")
/****************************   Modify end    *******************************************/
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = MONETX_HLT_MACRO_GC2385_SENSOR_ID,        /*record sensor id defined in Kd_imgsensor.h*/
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	...
	{1600, 1200, 160, 240, 1280, 720, 1280, 720, 0000, 0000, 1280, 720, 0,  0, 1280, 720} /* slim video */
};
...
static void set_dummy(void)
{
	...
	hb = imgsensor.dummy_pixel + GC2385_DEFAULT_DUMMY_PIXEL_NUMS;
	vb = imgsensor.dummy_line + Basic_Dummy_Line;
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 240)	/*240,	300 for test*/
		write_cmos_sensor(0xf8, 0x20);	/*24fps */
	else
		write_cmos_sensor(0xf8, 0x28);	/*30fps*/
	write_cmos_sensor(0xf9, 0x02);
	write_cmos_sensor(0xfa, 0x08);
	write_cmos_sensor(0xfc, 0x8e);
	write_cmos_sensor(0xe7, 0xcc);
	...
	write_cmos_sensor(0x25, 0x13);
	if (currefps == 240) { /*240,	300 for test*/
	write_cmos_sensor(0x26, 0x05);   /*24fps */
	write_cmos_sensor(0x2b, 0x05);
	} else {
	write_cmos_sensor(0x26, 0x05);   /*30fps*/
	write_cmos_sensor(0x2b, 0x05);
	}
	write_cmos_sensor(0x29, 0x06);
	write_cmos_sensor(0x2a, 0x08);
...
static void slim_video_setting(void)
{
    ...
    /*out crop*/
	write_cmos_sensor(0x90, 0x01);
	write_cmos_sensor(0x92, STARTY+0xf0);
	write_cmos_sensor(0x94, STARTX+0xa0);
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("Get GC2385 sensor success!i2c write id: 0x%x, sensor id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
#if ENABLE_GC2385_OTP
						...
						MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    /*ensor_info->SensorVideoFrameRate = imgsensor_info.normal_video.max_framerate/10; not use */
...
static kal_uint32 set_max_framerate_by_scenario(enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
		...
		spin_unlock(&imgsensor_drv_lock);
		set_dummy();
		Basic_Dummy_Line = imgsensor.min_frame_length - 1232;
		break;
	case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
		if (framerate == 0)
			return ERROR_NONE;
							...
							UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		LOG_INF("SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d,",
