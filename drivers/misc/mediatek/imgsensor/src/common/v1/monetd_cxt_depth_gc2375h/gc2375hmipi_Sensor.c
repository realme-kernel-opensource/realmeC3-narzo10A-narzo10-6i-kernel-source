...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = MONETD_CXT_DEPTH_GC2375H_SENSOR_ID,
	...
	.custom1 = {
		...
		.linelength = 1169,       /* record different mode's linelength */
		.framelength = 1497,
		.startx = 0,
		.starty = 0,
		.grabwindow_width = 1600,
...
static kal_uint32 custom1(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	printk("[gc2375] custom1 mode start\n");
	spin_lock(&imgsensor_drv_lock);
	...
	imgsensor.autoflicker_en = KAL_FALSE;
	spin_unlock(&imgsensor_drv_lock);
	custom1_setting();//custom1_setting(imgsensor.current_fps);
	return ERROR_NONE;
}
	...
	UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_PIXEL_RATE:
	{
		...
		case MSDK_SCENARIO_ID_SLIM_VIDEO:
			...
			break;
		case MSDK_SCENARIO_ID_CUSTOM1:
			rate = (imgsensor_info.custom1.pclk /
