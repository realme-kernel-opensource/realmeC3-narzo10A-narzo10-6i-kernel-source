...
extern int oppo_unregister_dhall(const char * name);
extern int oppo_register_motor(const char * name, struct oppo_motor_operations *ops);
extern int oppo_unregister_motor(const char * name);
//dhall control api
extern int oppo_dhall_get_data(unsigned int id, short *hall_val, bool raw);
...
extern bool oppo_dhall_update_threshold(unsigned int id, int position,short thresh);
extern bool oppo_dhall_is_power_on(void);
//motor control api
extern int oppo_motor_set_power (enum motor_state state, int mode);
...
extern int oppo_motor_pwm_config(int duty_ns, int period_ns, uint32_t wave_num);
...
extern int oppo_get_driver_ic_id(void);
extern int oppo_get_dir_sign(void);
extern bool oppo_is_support_mode(void);
#endif
