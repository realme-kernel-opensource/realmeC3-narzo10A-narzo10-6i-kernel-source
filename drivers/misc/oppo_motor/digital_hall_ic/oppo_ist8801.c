...
static struct oppo_dhall_chip *g_chip[DHALL_NUM] = {NULL,NULL};
...
__attribute__((weak)) void oppo_ist8801_reconfig(struct oppo_dhall_chip *chip) {return;}

static int probe_count = 0;
static int ist8801_i2c_read_block(struct oppo_dhall_chip *chip, u8 addr, u8 *data, u8 len)
{
	u8 reg_addr = addr;
	int err = 0;
...
static int ist8801_i2c_write_block(struct oppo_dhall_chip *chip, u8 addr, u8 *data, u8 len)
{
	int err = 0;
	int idx = 0;
	int num = 0;
...
static void ist8801_short_to_2byte(struct oppo_dhall_chip *chip, short x, u8 *hbyte, u8 *lbyte)
{
	unsigned short temp;
...
static short ist8801_2byte_to_short(struct oppo_dhall_chip *chip, u8 hbyte, u8 lbyte)
{
	short x = 0;
...
static int ist8801_get_id(struct oppo_dhall_chip *chip)
{
	u8 data = 0;
...
static int ist8801_set_power_gpio_down(struct oppo_dhall_chip * chip)
{
	int ret = 0;
...
static int ist8801_set_power_gpio_up(struct oppo_dhall_chip * chip)
{
	int ret = 0;
...
static int ist8801_set_power(struct oppo_dhall_chip *chip, bool on)
{
	int ret = 0;
...
/*
static int ist8801_set_frequency(struct oppo_dhall_chip *chip,int frequency)
{
        int err = 0;
        u8 rdata = 0,ifcntl = 0;
...
*/
static int ist8801_clear_interrupt(struct oppo_dhall_chip *chip)
{
    int ret = 0;
...
*/
static bool ist8801_update_threshold(unsigned int id, int position, short lowthd, short highthd)
{
...
static int ist8801_set_operation_mode(struct oppo_dhall_chip *chip, int mode)
{
	u8 opf = 0,ifcntl = 0;
	int ret = 0;
...
static irqreturn_t ist8801_irq0_handler(int irq, void *dev_id)
{
	...
	oppo_dhall_irq_handler(DHALL_0);
...
static irqreturn_t ist8801_irq1_handler(int irq, void *dev_id)
{
	...
	oppo_dhall_irq_handler(DHALL_1);

	return IRQ_HANDLED;
}

int ist8801_setup_eint(struct oppo_dhall_chip *chip)
{
	int ret = 0;
...
static int ist8801_get_irq_state(unsigned int id)
{
	...
	return ((g_chip[id]->reg.map.intsrs & IST8801_DETECTION_MODE_INTERRUPT) ? 1 : 0);
}
...
static void ist8801_set_sensitivity(unsigned int id, char *value)
{
	int i = 0;
	uint8_t rwdata;
	struct oppo_dhall_chip *chip = g_chip[id];
...
16-bit: other
*/
static int ist8801_reset_device(struct oppo_dhall_chip *chip)
{
	int err = 0;
	u8 data =0;
	...
	if (data!= IST8801_VAL_DID) {
				MOTOR_ERR("current device id(0x%02X) is not IST8801 device id(0x%02X)",data,IST8801_VAL_DID);
...
static void ist8801_parse_dts(struct oppo_dhall_chip * chip)
{
...
struct oppo_dhall_operations  ist8801_ops = {
...
{
	struct oppo_dhall_chip *chip = NULL;
	u8 dev_id = 0xFF;
	int err = 0;
	...
	chip = devm_kzalloc(&client->dev,sizeof(struct oppo_dhall_chip), GFP_KERNEL);
	if (!chip) {
	...
	oppo_register_dhall("ist8801",&ist8801_ops);
