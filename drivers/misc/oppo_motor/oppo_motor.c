...
static struct oppo_motor_chip * g_chip = NULL;
...
__attribute__((weak)) void oppo_parse_motor_info(struct oppo_motor_chip * chip)
{
...
static int oppo_motor_get_speed(int index, unsigned int *speed)
{
...
int oppo_unregister_dhall(const char * name)
{
	if (g_chip) {
...
int oppo_unregister_motor(const char * name)
{
	if (g_chip) {
...
int oppo_dhall_set_SRS(unsigned int id, char *value)
{
...
int oppo_dhall_get_data(unsigned int id, short *hall_val, bool raw)
{
...
static void oppo_dhall_clear_max_data(struct oppo_motor_chip *chip)
{
	chip->hall_max.data0 = -512;
	chip->hall_max.data1 = -512;
}

static void oppo_dhall_record_max_data(short hall0_val, short hall1_val)
{
	if (!g_chip) {
		return;
	}
...
bool oppo_dhall_update_threshold(unsigned int id, int position, short thresh)
{
	short thresh_low = 0;
	short thresh_high = 0;
	...
	return g_chip->dhall_ops->update_threshold(id, position, thresh_low, thresh_high);
}
...
bool oppo_dhall_is_power_on(void)
{
	if (!g_chip || !g_chip->dhall_ops || !g_chip->dhall_ops->is_power_on) {
		...
		if (g_chip->dhall_ops->is_power_on(DHALL_0) || g_chip->dhall_ops->is_power_on(DHALL_1))
			return true;
		else
			return false;
	}
...
int oppo_motor_set_power (enum motor_state state, int mode)
{
...
int oppo_motor_pwm_config(int duty_ns, int period_ns, uint32_t wave_num)
{
...
int oppo_get_motor_type(void)
{
	if (!g_chip ) {
		...
		if (g_chip->info.type == MOTOR_UNKNOWN)
			oppo_parse_motor_info(g_chip);
		return g_chip->info.type;
	}

}

bool oppo_is_support_mode(void)
{
	if (!g_chip) {
		return false;
	} else {
...
int oppo_get_driver_ic_id(void)
{
	if (!g_chip ) {
		return DRV8834;
	} else {
...
static void oppo_change_dhall_sign(short hall0, short hall1)
{
	if (!g_chip ) {
...
static bool check_normal_strong_mag(enum motor_direction dir, short hall0_val, short hall1_val)
{
	...
	if ((hall0_val < g_chip->noise.hall0_weak && hall0_val > -g_chip->noise.hall0_weak) &&
		(hall1_val < g_chip->noise.hall1_weak && hall1_val > -g_chip->noise.hall1_weak)) {
		return false;
	}
...
static void irq_pos_update(struct oppo_motor_chip *chip, unsigned int id, bool makeup)
{
	if (!chip) {
	...
	if (id == DHALL_0) {
		if (makeup) {
			chip->cali_data.irq_pos_hall0 = chip->cali_data.irq_pos_hall0_bak + chip->cali_data.irq_makeup_hall0;
		} else {
			chip->cali_data.irq_pos_hall0 = chip->cali_data.irq_pos_hall0_bak;
		}
	} else if (id == DHALL_1) {
		if (makeup) {
			chip->cali_data.irq_pos_hall1 = chip->cali_data.irq_pos_hall1_bak + chip->cali_data.irq_makeup_hall1;
		} else {
			chip->cali_data.irq_pos_hall1 = chip->cali_data.irq_pos_hall1_bak;
		}
	}
	MOTOR_LOG("update irq_hall0:%d, irq_hall1:%d.\n", chip->cali_data.irq_pos_hall0, chip->cali_data.irq_pos_hall1);
}
...
int oppo_dhall_irq_handler(unsigned int id)
{
	...
	oppo_dhall_get_data(DHALL_0, &g_chip->noise.pre_hall0, false);
	oppo_dhall_get_data(DHALL_1, &g_chip->noise.pre_hall1, false);

	g_chip->irq_count[id]++;

	if (id == DHALL_0) {
		queue_delayed_work(g_chip->manual2auto_wq, &g_chip->up_work, msecs_to_jiffies(50));
	} else if (id == DHALL_1) {
...
static void oppo_set_md_mode_para(int md_mode)
{
	...
	g_chip->pwm_param.normal.full.pwm = oppo_motor_calculate_pwm_count(g_chip->pwm_param.normal.full.L, mode);
	g_chip->pwm_param.normal.nd.pwm = oppo_motor_calculate_pwm_count(g_chip->pwm_param.normal.nd.L, mode);
	g_chip->pwm_param.normal.speed_up.pwm = oppo_motor_calculate_pwm_count(g_chip->pwm_param.normal.speed_up.L, mode);
	g_chip->pwm_param.normal.speed_down.pwm = oppo_motor_calculate_pwm_count(g_chip->pwm_param.normal.speed_down.L, mode);
	...
	g_chip->pwm_param.slow.full.pwm = oppo_motor_calculate_pwm_count(g_chip->pwm_param.slow.full.L, mode);
	g_chip->pwm_param.slow.nd.pwm = oppo_motor_calculate_pwm_count(g_chip->pwm_param.slow.nd.L, mode);
	g_chip->pwm_param.slow.speed_up.pwm = oppo_motor_calculate_pwm_count(g_chip->pwm_param.slow.speed_up.L, mode);
	g_chip->pwm_param.slow.speed_down.pwm = oppo_motor_calculate_pwm_count(g_chip->pwm_param.slow.speed_down.L, mode);
...
static int oppo_update_speed_para(bool slow)
{
	if (slow) {
		...
		g_chip->pwm_param.run.speed_down.speed = g_chip->pwm_param.slow.speed_down.speed;
		...
		g_chip->pwm_param.run.down_brake_delay = g_chip->pwm_param.slow.down_brake_delay;
	} else {
		...
		g_chip->pwm_param.run.speed_down.speed = g_chip->pwm_param.normal.speed_down.speed;
		...
		g_chip->pwm_param.run.down_brake_delay = g_chip->pwm_param.normal.down_brake_delay;
	}

	return 0;
};

static int oppo_update_motor_speed(unsigned int speed_ns)
{
...
static void oppo_parameter_init(struct oppo_motor_chip *chip)
{
	...
	oppo_dhall_clear_max_data(chip);
	oppo_parse_motor_info(chip);
	if (chip->boot_mode != NORMAL_MODE) {
		MOTOR_ERR("no normal mode, skip parameter init.\n");
		return;
	}

	oppo_dhall_update_threshold(DHALL_0, DOWN_STATE, HALL_THRESH_HIGH);
	oppo_dhall_update_threshold(DHALL_1, DOWN_STATE, HALL_THRESH_HIGH);
	...
	oppo_update_motor_speed(chip->pwm_param.normal.speed_up.speed);
	oppo_motor_pwm_config(g_chip->pwm_param.pwm_duty, g_chip->pwm_param.pwm_period, 0);
...
static int oppo_motor_run_check(struct oppo_motor_chip *chip, enum start_mode mode)
{
	...
	oppo_dhall_get_data(DHALL_0, &hall0_val, false);
	oppo_dhall_get_data(DHALL_1, &hall1_val, false);
	...
	if ((mode == USER_START) && check_normal_strong_mag(chip->md_dir, hall0_val, hall1_val)) {
...
static void oppo_motor_control(enum start_mode mode, int dir)
{
	int ret = 0;
	...
	if (mode) {
		...
		if (!g_chip->motor_started) {
			...
			oppo_set_direction_para(dir);
			ret = oppo_motor_run_check(g_chip, mode);
			if (!ret) {
...
static void oppo_motor_start(enum start_mode mode, int dir)
{
	bool slow = false;
	static int retry = 0;
	...
	oppo_update_speed_para(slow);

	oppo_motor_control(mode, dir);
}

static void oppo_motor_stop(void)
{
	oppo_motor_control(STOP, 0);
}

static void oppo_motor_change_speed(L_param_t *speed_param)
{
	...
	if (oppo_update_motor_speed(speed)) {
...
static void oppo_motor_upward(void)
{
	...
	oppo_motor_start(USER_START, MOTOR_UPWARD);
}

void oppo_motor_downward(enum start_mode mode)
{
	if (!g_chip) {
		MOTOR_LOG("g_chip null \n ");
		return;
	...
	oppo_motor_start(mode, MOTOR_DOWN);
}
...
static irqreturn_t oppo_free_fall_detect_handler(int irq, void *dev_id)
{
	...
	oppo_motor_downward(USER_START);
...
static void oppo_motor_parse_dts(struct oppo_motor_chip * chip)
{
	...
	chip->motor_retard_offset_support = of_property_read_bool(node, "motor_retard_offset_support");
	chip->extre_stop_support = of_property_read_bool(node, "extre_stop_support");
	chip->motor_stop_val_support = of_property_read_bool(node, "motor_stop_val_support");
	...
	chip->motor_small_irq_later_speed_down_support = of_property_read_bool(node, "motor_small_irq_later_speed_down_support");

	rc = of_property_read_u32_array(node, "small-irq-later-speed-down", small_irq_tmp,3);
	if (rc) {
		chip->small_irq = 0;
	...
	chip->cal_fix_stop_retry_support = of_property_read_bool(node, "cal_fix_stop_retry_support");
	...
	chip->motor_down_stop_support = of_property_read_bool(node, "motor_down_stop_support");
	...
	chip->down_state_high_thresh_support = of_property_read_bool(node, "down_state_high_thresh_support");
...
static void oppo_motor_free_fall_register(struct oppo_motor_chip * chip)
{
	...
	if (!gpio_is_valid(chip->free_fall_gpio)) {
		...
		ret = request_threaded_irq(chip->irq, NULL, &oppo_free_fall_detect_handler,
...
static enum alarmtimer_restart motor_reset_timer_func(struct alarm *alrm, ktime_t now)
{
	...
	MOTOR_LOG("irq_count [%d %d] \n",g_chip->irq_count[DHALL_0], g_chip->irq_count[DHALL_1]);
...
static int fb_notifier_callback(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	if (event == MSM_DRM_EARLY_EVENT_BLANK) {
		...
		if (blank == MSM_DRM_BLANK_UNBLANK) {
			...
			oppo_motor_set_power(WAKE_ACTIVE, MOTOR_POWER_IGNORE);
		}
	}
...
static ktime_t motor_cal_full_timer(bool mag)
{
	...
	if (g_chip->pwm_param.run.speed_up.L) {
		value += (long long)g_chip->pwm_param.run.speed_up.pwm*(long long)g_chip->pwm_param.run.speed_up.speed;
	}

	if (g_chip->pwm_param.run.nd.L) {
		value += (long long)g_chip->pwm_param.run.nd.pwm*(long long)g_chip->pwm_param.run.nd.speed;
	}

	if (g_chip->pwm_param.run.full.L) {
		value += (long long)g_chip->pwm_param.run.full.pwm*(long long)g_chip->pwm_param.run.full.speed;
	}

	if (g_chip->pwm_param.run.speed_down.L) {
		value += (long long)g_chip->pwm_param.run.speed_down.pwm*(long long)g_chip->pwm_param.run.speed_down.speed;
	}
...
static void motor_run_work(struct work_struct *work)
{
	...
	if (chip->motor_enable && (chip->motor_started == 0)) {
		...
		oppo_motor_set_power(STAY_ACTIVE, MOTOR_POWER_ON);
		...
		oppo_update_motor_speed(chip->pwm_param.run.speed_up.speed);

		ret = oppo_motor_pwm_config(chip->pwm_param.pwm_duty, chip->pwm_param.pwm_period, 0);
		if (ret < 0) {
		...
		hrtimer_start(&chip->stop_timer, motor_cal_full_timer(chip->strong_mag), HRTIMER_MODE_REL);

		//calculate when the motor should speed up
		pwm = chip->pwm_param.run.speed_up.pwm;
		speed = chip->pwm_param.pwm_period;
		hrtimer_start(&chip->speed_up_timer, motor_cal_timer(pwm, speed), HRTIMER_MODE_REL);
		...
		oppo_motor_pwm_disable();
		oppo_motor_set_power(STAY_ACTIVE, MOTOR_POWER_OFF);
		...
		if (chip->move_state == MOTOR_UPWARD_ING) {
			...
			oppo_dhall_get_data(DHALL_1, &hall1_val, false);
			if (hall1_val > chip->cali_data.irq_pos_hall1 || chip->strong_mag) { // if strong mag, set normal
				chip->position = UP_STATE;
				MOTOR_ERR("POS_NORMAL, irq_count1 %d\n",chip->irq_count[DHALL_1]);
				chip->irq_count[DHALL_1] = 0;
				oppo_dhall_update_threshold(DHALL_1, UP_STATE, chip->cali_data.irq_pos_hall1);
		...
		} else if (chip->move_state == MOTOR_DOWNWARD_ING) { // if strong mag, set normal
			...
			if (hall0_val > chip->cali_data.irq_pos_hall0 || chip->strong_mag) {
				...
				if (chip->down_state_high_thresh_support && hall0_val > chip->cali_data.down_retard_hall0) {
					MOTOR_ERR("update threshold to HALL_THRESH_HIGH\n");
					oppo_dhall_update_threshold(DHALL_0, DOWN_STATE, HALL_THRESH_HIGH);
				} else {
					oppo_dhall_update_threshold(DHALL_0, DOWN_STATE, chip->cali_data.irq_pos_hall0);
				}
...
static void manual_to_auto_up_work(struct work_struct *work)
{
	...
	oppo_dhall_update_threshold(DHALL_0, MID_STATE, HALL_THRESH_HIGH);
...
static void manual_to_auto_down_work(struct work_struct *work)
{
	...
	oppo_dhall_update_threshold(DHALL_1, MID_STATE, HALL_THRESH_HIGH);

	if (chip->manual2auto_down_switch) {
		if (!chip->motor_started) {
			report_positon_state_manual_to_down(chip);
			oppo_motor_downward(IRQ_START);
		}
	}
...
static bool extre_stop_check(struct oppo_motor_chip *chip, short hall1_val, short hall1_val_pre, short hall0_val, short hall0_val_pre)
{
	if ((chip->move_state == MOTOR_UPWARD_ING) && (abs(hall1_val_pre) > abs(hall1_val))) {
		MOTOR_ERR("moving up stop by hall1 < hall1_pre.\n");
		return true;
	}

	if ((chip->move_state == MOTOR_DOWNWARD_ING) && (abs(hall0_val_pre) > abs(hall0_val))
...
static bool hall_valid_check(struct oppo_motor_chip *chip, short hall1_val, short hall1_val_pre, short hall0_val, short hall0_val_pre)
{
	//prevent one byte to two byte, hall value can't sync
	if ((chip->move_state == MOTOR_DOWNWARD_ING) && (hall0_val > 2*hall0_val_pre) && (hall0_val_pre > VALID_CHECK_THD))
		return false;
	if ((chip->move_state == MOTOR_UPWARD_ING) && (hall1_val > 2*hall1_val_pre) && (hall1_val_pre > VALID_CHECK_THD))
		return false;
...
static void position_detect_work(struct work_struct *work)
{
	...
	oppo_motor_change_speed(&chip->pwm_param.run.nd);

	/*delay position detect*/
	pwm_count = oppo_motor_calculate_pwm_count(chip->pwm_param.run.nd.L, 0);
	...
	while (chip->motor_started) {
		...
		if (usecond && ((chip->md_dir == MOTOR_DOWN  && hall0_val < chip->cali_data.irq_pos_hall0) ||
		...
		if (!switch2fullspeed) {
			...
			oppo_motor_change_speed(&chip->pwm_param.run.full);
		...
		if(chip->down_data_buf != NULL && chip->up_data_buf != NULL) {
			if((chip->md_dir == MOTOR_DOWN) && (MAX_BUF_LEN-len-1 > 0)) {
				len += snprintf(chip->down_data_buf + len, MAX_BUF_LEN-len-1, "[%4d]\t[%4d]\t[%4d]\t[%4d]\t[DOWN]\n",
					hall0_val, hall1_val, hall_delta, delta_d);
			} else if((chip->md_dir == MOTOR_UPWARD) && (MAX_BUF_LEN-len-1 > 0)) {
				len += snprintf(chip->up_data_buf + len, MAX_BUF_LEN-len-1, "[%4d]\t[%4d]\t[%4d]\t[%4d]\t[UPWARD]\n",
					hall0_val, hall1_val, hall_delta, delta_d);
			}
		}
		oppo_dhall_record_max_data(hall0_val,hall1_val);
		...
		if (speed_down && !reset_retry && !chip->stop.offset[0] && !chip->stop.offset[1]) {
			reset_retry = true;
		...
		if (chip->motor_small_irq_later_speed_down_support && chip->cali_data.irq_pos_hall0 < chip->small_irq) {
		...
		if (hall0_val > down_retard_hall0) {
			if ((chip->move_state == MOTOR_DOWNWARD_ING) && (speed_down == 0)) {
				...
				if (chip->motor_retard_offset_support) {
					...
					oppo_motor_change_speed(&chip->pwm_param.run.speed_down);
				}
				//oppo_motor_change_speed(&chip->pwm_param.run.speed_down);
		...
		if (hall0_val < chip->cali_data.irq_pos_hall0 && hall1_val < chip->cali_data.irq_pos_hall1) {
			...
			if ((chip->stop.offset[0] > 0) && (down_stop_offset > 0)) {
				hall0_cali_diff = chip->run_check_hall0 - down_retard_hall0 - chip->stop.cali_retard_offset[1];
				if ((hall0_cali_diff > chip->stop.hall_max_thrd[1]) && (chip->move_state == MOTOR_DOWNWARD_ING)) {
				...
				} else if (hall1_val >= (chip->cali_data.up_retard_hall1 + chip->stop.offset[0])) {
					if (chip->move_state == MOTOR_UPWARD_ING) {
						stop_retry = chip->offset_stop_retry;
			...
			} else if ((chip->stop.pos_change_thrd[0] > 0) && (chip->stop.pos_change_thrd[1] > 0)) {
		...
		if ((chip->move_state == MOTOR_DOWNWARD_ING) && chip->cal_fix_stop_retry_support && hall0_val >= (down_retard_hall0)) {
			stop_retry = 1;
		...
		if (hall_valid_check(chip, hall1_val, hall1_val_pre, hall0_val, hall0_val_pre)) {
			   ...
			   const char *buff, size_t count)
{
	...
	if (sscanf(buff, "%d,%d,%d", &md_index, &brake_index, &pulse_index) == 3) {
		if (oppo_motor_get_speed(md_index, &g_chip->pwm_param.normal.full.speed)) {
			MOTOR_ERR("failed to set md speed\n");
		}

		if (oppo_motor_get_speed(md_index, &g_chip->pwm_param.normal.nd.speed)) {
			MOTOR_ERR("failed to set nd speed\n");
		}

		if (oppo_motor_get_speed(brake_index, &g_chip->pwm_param.normal.speed_down.speed)) {
			MOTOR_ERR("failed to set brake speed\n");
		}

		if (oppo_motor_get_speed(pulse_index, &g_chip->pwm_param.normal.speed_up.speed)) {
...
static ssize_t motor_speed_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	do {
		if (!oppo_motor_get_speed(index, &speed)) {
			c += snprintf(buf + c, PAGE_SIZE, "%d.%dkHZ:%d ", 1000000/speed, (100*1000000/speed)%100, index);
		} else {
			break;
		}
	} while (index++ < 20);
					...
					const char *buff, size_t count)
{
	...
	if (sscanf(buff, "%d", &enable) == 1) {
		if (enable) {
			MOTOR_LOG("oppo_motor_start %d\n", enable);
			oppo_motor_start(enable, g_chip->md_dir);
		} else {
			oppo_motor_stop();
		}
	}
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%s\n%s\n",g_chip->up_data_buf, g_chip->down_data_buf);
}
...
{
	struct oppo_motor_chip *chip = container_of(work, struct oppo_motor_chip, force_work);
...
static enum hrtimer_restart motor_force_move_stop_timer_func(struct hrtimer *hrtimer)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
		return HRTIMER_NORESTART;
...
static ssize_t motor_force_move_store(struct device *pdev, struct device_attribute *attr, const char *buff, size_t count)
{
	int enable = 0;
	int pwm_count = 0;
	...
	if (sscanf(buff, "%d", &enable) == 1) {
		...
		if (!g_chip->force_moving) {
			...
			oppo_update_motor_speed(g_chip->pwm_param.normal.full.speed);
			...
			oppo_motor_set_power(STAY_ACTIVE, MOTOR_POWER_ON);
			oppo_motor_set_direction(g_chip->md_dir);
			oppo_motor_pwm_config(g_chip->pwm_param.pwm_duty, g_chip->pwm_param.pwm_period, pwm_count);
			oppo_motor_pwm_enable();

			hrtimer_start(&g_chip->force_move_timer, motor_cal_timer(pwm_count, g_chip->pwm_param.pwm_period), HRTIMER_MODE_REL);
		}
	}
...
static ssize_t dhall_data_store (struct device *pdev, struct device_attribute *attr, const char *buff, size_t count)
{
	int id = 0;
	char value[16] = {0};
...
static ssize_t dhall_data_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	oppo_dhall_get_data(DHALL_0, &hall0_val, true);
	oppo_dhall_get_data(DHALL_1, &hall1_val, true);
...
static ssize_t dhall_max_data_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	short hall0_val, hall1_val;
	...
	MOTOR_ERR("[%d,%d,%d,%d]\n", hall0_val, hall1_val, g_chip->hall_max.data0, g_chip->hall_max.data1);

	if (g_chip->position == DOWN_STATE) {
		return sprintf(buf, "%d,%d\n", g_chip->hall_max.data0 * g_chip->sign, hall1_val);
	} else if (g_chip->position == UP_STATE) {
		return sprintf(buf, "%d,%d\n", hall0_val, g_chip->hall_max.data1 * g_chip->sign);
	} else {
		return snprintf(buf, PAGE_SIZE, "%d,%d\n",0,0);
	}
}
...
static ssize_t dhall_all_reg_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	oppo_dhall_dump_regs(DHALL_0, _buf0);
	oppo_dhall_dump_regs(DHALL_1, _buf1);
...
static ssize_t motor_manual2auto_switch_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if ((g_chip->manual2auto_up_switch == 1) || (g_chip->manual2auto_down_switch == 1))
	...
	if (sscanf(buff, "%d", &data[0]) == 1) {
		...
		if (data[0] == 1) {
			if (g_chip->position == UP_STATE) {
				oppo_dhall_update_threshold(DHALL_1, MID_STATE, g_chip->cali_data.irq_pos_hall1);
			} else if (g_chip->position == DOWN_STATE) {
				oppo_dhall_update_threshold(DHALL_0, MID_STATE, g_chip->cali_data.irq_pos_hall0);
			}
...
static ssize_t dhall_stop_val_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
		return snprintf(buf, PAGE_SIZE, "%d\n%d\n",-1,-1);
...
static ssize_t dhall_stop_val_store (struct device *pdev, struct device_attribute *attr, const char *buff, size_t count)
{
	int stop_value[2] = {0};
...
static ssize_t dhall_in_calibration_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
		return snprintf(buf, PAGE_SIZE, "%d\n",0);
...
static ssize_t dhall_in_calibration_store (struct device *pdev, struct device_attribute *attr, const char *buff, size_t count)
{
						...
						const char *buff, size_t count)
{
	...
	if (sscanf(buff, "%d,%d,%d,%d,%d,%d", &data[0],&data[1],&data[2],&data[3],&data[4],&data[5]) == 6) {
		...
		oppo_change_dhall_sign(data[2], data[5]);
...
static ssize_t dhall_distance_store(struct device *pdev, struct device_attribute *attr,
						const char *buff, size_t count)
{
	...
	if (sscanf(buff, "%d,%d", &up_distance, &down_distance) == 2) {
		...
		oppo_set_md_mode_para(g_chip->pwm_param.md_mode);
	}
...
static ssize_t dhall_stop_range_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
		return snprintf(buf, PAGE_SIZE, "up range <%d, %d> down range <%d, %d>",0,0,0,0);
	}
...
static ssize_t dhall_stop_range_store(struct device *pdev, struct device_attribute *attr,
						const char *buff, size_t count)
{
...
static ssize_t motor_detect_delay_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
		return snprintf(buf, PAGE_SIZE, "%d\n%d\n%d\n",-1,-1,-1);
...
static ssize_t motor_detect_delay_store(struct device *pdev, struct device_attribute *attr,
						const char *buff, size_t count)
{
	int delay[3] = {0};
...
static ssize_t motor_terminal_offset_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
		return snprintf(buf, PAGE_SIZE, "%d\n",0);
...
static ssize_t motor_terminal_offset_store(struct device *pdev, struct device_attribute *attr,
						const char *buff, size_t count)
{
	int offset[2] = {0};
...
static ssize_t motor_stop_in_advance_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
		return snprintf(buf, PAGE_SIZE, "%d\n",0);
...
static ssize_t motor_stop_in_advance_store(struct device *pdev, struct device_attribute *attr,
						const char *buff, size_t count)
{
	int val = 0;
...
static ssize_t small_irq_later_speed_down_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
		return snprintf(buf, PAGE_SIZE, "%d\n",0);
	...
	return sprintf(buf, "<%d,%d,%d>\n",g_chip->small_irq, g_chip->later_speed_down, g_chip->later_speed_down_stop_offset);
}

static ssize_t small_irq_later_speed_down_store(struct device *pdev, struct device_attribute *attr,
						const char *buff, size_t count)
{
	int tmp[3] = {0};
...
static DEVICE_ATTR(stop_range,	 S_IRUGO | S_IWUSR, dhall_stop_range_show, dhall_stop_range_store);
...
static DEVICE_ATTR(delay, S_IRUGO | S_IWUSR, motor_detect_delay_show, motor_detect_delay_store);
static DEVICE_ATTR(terminal_offset, S_IRUGO | S_IWUSR, motor_terminal_offset_show, motor_terminal_offset_store);
static DEVICE_ATTR(hall_data,	S_IRUGO | S_IWUSR, dhall_data_show, dhall_data_store);
static DEVICE_ATTR(hall_reg,   S_IRUGO | S_IWUSR, dhall_all_reg_show,NULL);
static DEVICE_ATTR(hall_max_data,   S_IRUGO | S_IWUSR, dhall_max_data_show,NULL);
...
static DEVICE_ATTR(hall_get_data,	 S_IRUGO | S_IWUSR,dhall_get_data_show,NULL);
static DEVICE_ATTR(in_calibration,   S_IRUGO | S_IWUSR, dhall_in_calibration_show, dhall_in_calibration_store);
static DEVICE_ATTR(stop_in_advance,   S_IRUGO | S_IWUSR, motor_stop_in_advance_show, motor_stop_in_advance_store);
static DEVICE_ATTR(stop_val,   S_IRUGO | S_IWUSR, dhall_stop_val_show, dhall_stop_val_store);
static DEVICE_ATTR(small_irq_later_speed_down,   S_IRUGO | S_IWUSR, small_irq_later_speed_down_show,small_irq_later_speed_down_store);
...
static long motor_ioctl(struct file* file, unsigned int cmd, unsigned long arg)
{
	...
	case MOTOR_IOCTL_START_MOTOR:
		...
		if (data)
			oppo_motor_start(data, g_chip->md_dir);;
		break;
...
{
	MOTOR_LOG("call \n");
	oppo_motor_set_power(DEEP_SLEEP, MOTOR_POWER_IGNORE);
