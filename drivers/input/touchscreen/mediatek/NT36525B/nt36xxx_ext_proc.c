...
static ssize_t oppo_tp_limit_enable_write(struct file *filp, const char __user *buf,
		size_t count, loff_t *ppos)
{
	unsigned int tmp = 0;
	char cmd[128] = {0};
...
static ssize_t oppo_tp_limit_enable_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	ssize_t ret = 0;
	char *ptr = NULL;
	int lens = 0;
...
/*oppo_tp_direction*/
static ssize_t oppo_tp_direction_write(struct file *filp, const char __user *buf,
		size_t count, loff_t *ppos)
{
	unsigned int tmp = 0;
	char cmd[128] = {0};
...
static ssize_t oppo_tp_direction_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	ssize_t ret = 0;
	uint8_t len;
	char *ptr = NULL;
	...
	*ppos +=count;
	...
	if(ts->is_suspended == 1){
		NVT_ERR("oppo_tp_direction read : the NT device is already suspend\n");
		return -1;
	}
...
static ssize_t oppo_tp_hop_test_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	ssize_t ret = 0;
	uint8_t len;
	char *ptr = NULL;
	...
	*ppos +=count;
	...
	if(ts->is_suspended == 1){
		NVT_ERR("oppo_tp_hop_test read : the NT device is already suspend\n");
		return -1;
	}
...
static const struct file_operations oppo_tp_limit_enable_fops =
{
	...
	.read = oppo_tp_limit_enable_read,
	.owner = THIS_MODULE,
};

static const struct file_operations oppo_tp_direction_fops =
{
	.write = oppo_tp_direction_write,
	.read = oppo_tp_direction_read,
	.owner = THIS_MODULE,
};
...
static ssize_t oppo_tp_limit_area_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	...
	len += snprintf(ptr+len, count-len,"left_u(x1,y1)=(%d,%d)    ",ts->nvt_oppo_proc_data->nvt_limit_area.area_xlu,
		ts->nvt_oppo_proc_data->nvt_limit_area.area_ylu);

	len += snprintf(ptr+len, count-len,"right_u(x2,y2)=(%d,%d)\n",ts->nvt_oppo_proc_data->nvt_limit_area.area_xru,
		ts->nvt_oppo_proc_data->nvt_limit_area.area_yru);

	len += snprintf(ptr+len, count-len,"left_b(x1,y1)=(%d,%d)    ",ts->nvt_oppo_proc_data->nvt_limit_area.area_xlb,
		ts->nvt_oppo_proc_data->nvt_limit_area.area_ylb);

	len += snprintf(ptr+len, count-len,"right_b(x2,y2)=(%d,%d)\n",ts->nvt_oppo_proc_data->nvt_limit_area.area_xrb,
		ts->nvt_oppo_proc_data->nvt_limit_area.area_yrb);
...
static ssize_t oppo_tp_limit_area_write(struct file *filp, const char __user *buffer,size_t count, loff_t *ppos)
{
    char buf[8] = {0};
    int  temp;
	if (buffer != NULL)
	...
	ts->nvt_oppo_proc_data->nvt_limit_area.limit_area = temp;
    ts->nvt_oppo_proc_data->nvt_limit_area.area_xlu   = (ts->nvt_oppo_proc_data->nvt_limit_area.limit_area*1000)/100;
    ts->nvt_oppo_proc_data->nvt_limit_area.area_xru   = TOUCH_DEFAULT_MAX_WIDTH - ts->nvt_oppo_proc_data->nvt_limit_area.area_xlu;
    ts->nvt_oppo_proc_data->nvt_limit_area.area_xlb    = 2 * ts->nvt_oppo_proc_data->nvt_limit_area.area_xlu;
    ts->nvt_oppo_proc_data->nvt_limit_area.area_xrb   = TOUCH_DEFAULT_MAX_WIDTH - (2 * ts->nvt_oppo_proc_data->nvt_limit_area.area_xlu);

    NVT_LOG("limit_area = %d; left_x1 = %d; right_x1 = %d; left_x2 = %d; right_x2 = %d\n",
           ts->nvt_oppo_proc_data->nvt_limit_area.limit_area, ts->nvt_oppo_proc_data->nvt_limit_area.area_xlu,
           ts->nvt_oppo_proc_data->nvt_limit_area.area_xru,ts->nvt_oppo_proc_data->nvt_limit_area.area_xlb, ts->nvt_oppo_proc_data->nvt_limit_area.area_xrb);

    ts->nvt_oppo_proc_data->nvt_limit_area.area_ylu   = (ts->nvt_oppo_proc_data->nvt_limit_area.limit_area*1000)/100;
    ts->nvt_oppo_proc_data->nvt_limit_area.area_yru   =  ts->nvt_oppo_proc_data->nvt_limit_area.area_ylu;
    ts->nvt_oppo_proc_data->nvt_limit_area.area_ylb   = TOUCH_DEFAULT_MAX_HEIGHT-2 * ts->nvt_oppo_proc_data->nvt_limit_area.area_ylu;
    ts->nvt_oppo_proc_data->nvt_limit_area.area_yrb   = TOUCH_DEFAULT_MAX_HEIGHT - (2 * ts->nvt_oppo_proc_data->nvt_limit_area.area_ylu);
...
{
	.write = oppo_tp_limit_area_write,
	.read = oppo_tp_limit_area_read,
	.owner = THIS_MODULE,
};
...
#define NVT_MK_PROC_DIR(name, parent) \
			ts->nvt_oppo_proc_data->name##_dir_entry = proc_mkdir(#name, parent); \
			if (IS_ERR(ts->nvt_oppo_proc_data->name##_dir_entry)) { \
				NVT_ERR("Create proc dir entry '%s' failed %ld \n", \
			#name, PTR_ERR(ts->nvt_oppo_proc_data->name##_dir_entry)); \
				ts->nvt_oppo_proc_data->name##_dir_entry = NULL; \
				return PTR_ERR(ts->nvt_oppo_proc_data->name##_dir_entry); \
			}

#define NVT_MK_PROC_ENTRY(name, mode, parent) \
			ts->nvt_oppo_proc_data->name##_entry = proc_create(#name, mode, parent, &name##_fops); \
			if (IS_ERR(ts->nvt_oppo_proc_data->name##_entry)) { \
				NVT_ERR("Create proc entry '%s' failed %ld \n", \
			#name, PTR_ERR(ts->nvt_oppo_proc_data->name##_entry)); \
				ts->nvt_oppo_proc_data->name##_entry = NULL; \
				return PTR_ERR(ts->nvt_oppo_proc_data->name##_entry); \
			}
		...
		NVT_MK_PROC_ENTRY(irq_depath, 0666, ts->nvt_oppo_proc_data->touchpanel_dir_entry)
		NVT_MK_PROC_ENTRY(oppo_register_info, 0664, ts->nvt_oppo_proc_data->touchpanel_dir_entry)
	...
	oppo_irq_depath = proc_create(OPPO_IRQ_DEPATH, 0444, oppo_touchpanel_proc, &oppo_irq_depath_fops);
	...
	oppo_game_switch = proc_create(OPPO_GAME_SWITCH,0222, oppo_touchpanel_proc, &oppo_game_switch_fops);
	if (oppo_game_switch == NULL) {
        NVT_ERR("create proc/touchpanel/oppo_game_switch_enable Failed!\n");
		return -ENOMEM;
	}

	oppo_tp_limit_enable = proc_create(OPPO_TP_LIMIT_ENABLE,0222, oppo_touchpanel_proc, &oppo_tp_limit_fops);
