...
** Version: 1.0
** Date : 2018/12/14
** Author: yangyang2@ODM_WT.BSP.TP-FP.TP
**
** -------------------------Revision History:----------------------------------
...
#if LPGW_TEST
#define LPWG_RAWDATA_CSV_FILE "/sdcard/LPWGRawdataTest.csv"
#define LPWG_NOISE_CSV_FILE "/sdcard/LPWGNoiseTest.csv"
#endif
...
static void nvt_print_lmt_array(int32_t *array, int32_t x_ch, int32_t y_ch)
{
	...
	for (j = 0; j < y_ch; j++) {
		for(i = 0; i < x_ch; i++) {
			printk(KERN_CONT "%5d, ", array[j * x_ch + i]);
		}
		printk("\n");
	}
#if TOUCH_KEY_NUM > 0
	for (k = 0; k < Key_Channel; k++) {
		printk(KERN_CONT "%5d, ", array[y_ch * x_ch + k]);
	}
	printk("\n");
#endif /* #if TOUCH_KEY_NUM > 0 */
}
...
static int32_t nvt_save_rawdata_to_csv(int32_t *rawdata, uint8_t x_ch, uint8_t y_ch, const char *file_path, uint32_t offset)
{
	...
	for (y = 0; y < y_ch; y++) {
		for (x = 0; x < x_ch; x++) {
			...
			printk(KERN_CONT "%5d, ", rawdata[iArrayIndex]);
...
static int32_t c_show_selftest(struct seq_file *m, void *v)
{
	...
	if ((TestResult_Short == 0)
			...
			&& (TestResult_Noise == 0)) {
		if(ts->oppo_baseline_test_flag == 1) {
			seq_printf(m, "0 error(s). All test passed.\n");
		} else {
			seq_printf(m, "result=1\n");
		}

	} else {
		seq_printf(m, "result=0\n");
	}
...
/*
	if (ts->oppo_baseline_test_flag == 0) {
		...
		NVT_LOG("baseline test no need update mp fw, because the black screen test is following. this is the 36525B schedule\n");
	}
*/
	nvt_update_firmware(fw->firmware_name);
#endif
...
struct file_operations oppo_nvt_selftest_fops = {
	.owner = THIS_MODULE,
	...
	if ((!data) || (!len) || (len < size)) {
		...
		for (j = 0; j < Y_Channel; j++) {
			...
			for (i = 0; i < X_Channel; i++) {
				...
				printk(KERN_CONT "%5d, ", array[iArrayIndex]);
			}
			printk("\n");
		}
#if TOUCH_KEY_NUM > 0
...
int oppo_nvt_blackscreen_test(void)
{
...
/*
	if (ts->oppo_baseline_test_flag == 0) {
		NVT_LOG("can not do black screen test only\n");
		//nvt_update_firmware(fw->firmware_mp_name);
	}
*/
	nvt_rest_ddi();
	...
	if ((TestResult_FW_Rawdata == 0) || (TestResult_FW_Rawdata == 1)) {
		if (TestResult_FWMutual == 0) {
			NVT_LOG("FW Rawdata Test pass\n");
		}
	} else { // TestResult_FW_Rawdata is -1
		NVT_LOG(" FW Rawdata FAIL!\n");
	}
	...
	if ((TestResult_FWMutual == 0)
			&& (TestResult_FW_DiffMax == 0)) {
		sprintf(ts->nvt_oppo_proc_data->gesture_test.message, "0 error(s). All test passed");
	} else {
		if (ts->oppo_baseline_test_flag == 0) {
			sprintf(ts->nvt_oppo_proc_data->gesture_test.message, "can not do the test without baseline_test");
		} else {
			sprintf(ts->nvt_oppo_proc_data->gesture_test.message, "result=0");
		}
	}
...
{
	NVT_proc_selftest_entry = proc_create("nvt_selftest", 0444, NULL, &oppo_nvt_selftest_fops);
#if LPGW_TEST
