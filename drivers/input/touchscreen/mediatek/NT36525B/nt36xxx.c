...
#ifdef ODM_WT_EDIT
//Tongxing.Liu@ODM_WT.BSP.TP.FUNCTION.2019/10/07, add write hardwareinfo dmess.
...
struct upgrade_fw_info nvt_fw_list[] = {
    {HLT_BOE, "HLT", BOOT_UPDATE_FIRMWARE_HLT_BOE_NAME,  MP_UPDATE_FIRMWARE_HLT_BOE_NAME, up_fw_kernel_hlt_boe, sizeof(up_fw_kernel_hlt_boe), OPPO_BOOT_UPDATE_FIRMWARE_HLT_BOE_NAME},
    {HLT_BOE_GG3, "HLT", BOOT_UPDATE_FIRMWARE_HLT_BOE_GG3_NAME,  MP_UPDATE_FIRMWARE_HLT_BOE_GG3_NAME, up_fw_kernel_hlt_boe_gg3, sizeof(up_fw_kernel_hlt_boe_gg3), OPPO_BOOT_UPDATE_FIRMWARE_HLT_BOE_GG3_NAME},
};
...
info_retry:
	...
	if ( ts->vendor_id == 0 ){
	    ...
	    devinfo_info_tp_set(version, "HLT",OPPO_BOOT_UPDATE_FIRMWARE_HLT_BOE_NAME);
		//rmtp = register_tp_proc("tp",version, "HLT",OPPO_BOOT_UPDATE_FIRMWARE_NAME_HLT);
	} else {
		sprintf(Ctp_name,"HLT,NT525B,FW:0x%x\n",ts->fw_ver);
        sprintf(version,"HLT_NT525B_0x%02x",ts->fw_ver);
	    devinfo_info_tp_set(version, "HLT",OPPO_BOOT_UPDATE_FIRMWARE_HLT_BOE_GG3_NAME);
...
static bool nvt_corner_point_process(int i)
{
	...
	if (ts->nvt_oppo_proc_data->edge_limit.limit_00 == 0) {
		...
		if ((ts->nvt_oppo_proc_data->edge_limit.limit_lu) &&
		    ...
		    ts->nvt_oppo_proc_data->nvt_point_info[i].type  = NVT_AREA_CORNER;
			if (ts->nvt_oppo_proc_data->nvt_limit_area.which_area == NVT_AREA_NORMAL)
				return true;
		    ts->nvt_oppo_proc_data->nvt_coner_info[CORNER_TOPLEFT].id = i;
		    ts->nvt_oppo_proc_data->nvt_coner_info[CORNER_TOPLEFT].point_info = ts->nvt_oppo_proc_data->nvt_point_info[i];
		    ts->nvt_oppo_proc_data->nvt_coner_info[CORNER_TOPLEFT].flag = true;
			ts->nvt_oppo_proc_data->nvt_limit_area.which_area = ts->nvt_oppo_proc_data->nvt_point_info[i].type;
			flag = 1;
        ...
        if ((ts->nvt_oppo_proc_data->edge_limit.limit_ru)  &&
			...
			if (ts->nvt_oppo_proc_data->nvt_limit_area.which_area == NVT_AREA_NORMAL)
				return true;
			ts->nvt_oppo_proc_data->nvt_limit_area.which_area = ts->nvt_oppo_proc_data->nvt_point_info[i].type;
			ts->nvt_oppo_proc_data->nvt_coner_info[CORNER_TOPRIGHT].id = i;
			ts->nvt_oppo_proc_data->nvt_coner_info[CORNER_TOPRIGHT].point_info = ts->nvt_oppo_proc_data->nvt_point_info[i];
			ts->nvt_oppo_proc_data->nvt_coner_info[CORNER_TOPRIGHT].flag = true;
			flag = 1;
       ...
       if ((ts->nvt_oppo_proc_data->edge_limit.limit_lb) &&
	   	...
	   	ts->nvt_oppo_proc_data->nvt_point_info[i].y > ts->nvt_oppo_proc_data->nvt_limit_area.area_ylb)) {
			...
			if (ts->nvt_oppo_proc_data->nvt_limit_area.which_area == NVT_AREA_NORMAL)
				return true;
  			ts->nvt_oppo_proc_data->nvt_limit_area.which_area = ts->nvt_oppo_proc_data->nvt_point_info[i].type;
   			ts->nvt_oppo_proc_data->nvt_coner_info[CORNER_BOTTOMLEFT].id = i;
   			ts->nvt_oppo_proc_data->nvt_coner_info[CORNER_BOTTOMLEFT].point_info = ts->nvt_oppo_proc_data->nvt_point_info[i];
   			ts->nvt_oppo_proc_data->nvt_coner_info[CORNER_BOTTOMLEFT].flag = true;
			flag = 1;
       ...
       if ((ts->nvt_oppo_proc_data->edge_limit.limit_rb) &&
	   	...
	   	ts->nvt_oppo_proc_data->nvt_point_info[i].y > ts->nvt_oppo_proc_data->nvt_limit_area.area_yrb)) {
			...
			ts->nvt_oppo_proc_data->nvt_limit_area.which_area = ts->nvt_oppo_proc_data->nvt_point_info[i].type;
			ts->nvt_oppo_proc_data->nvt_coner_info[CORNER_BOTTOMRIGHT].id = i;
			ts->nvt_oppo_proc_data->nvt_coner_info[CORNER_BOTTOMRIGHT].point_info = ts->nvt_oppo_proc_data->nvt_point_info[i];
			ts->nvt_oppo_proc_data->nvt_coner_info[CORNER_BOTTOMRIGHT].flag = true;
			flag = 1;
        ...
        if (ts->nvt_oppo_proc_data->nvt_point_info[i].type != NVT_AREA_CORNER) {
            ...
            if (ts->nvt_oppo_proc_data->nvt_limit_area.which_area == NVT_AREA_CORNER) {
                for (j = 0; j < 4; j++) {
                    if (ts->nvt_oppo_proc_data->nvt_coner_info[j].flag) {
#ifdef MT_PROTOCOL_B
                        input_mt_slot(ts->input_dev, ts->nvt_oppo_proc_data->nvt_coner_info[j].id);
		...
		if ( flag == 0 ) {
			ts->nvt_oppo_proc_data->nvt_limit_area.which_area = NVT_AREA_NORMAL;
		} else {
			ts->nvt_oppo_proc_data->nvt_limit_area.which_area = NVT_AREA_CORNER;
		}
    }
...
//Bin.Su@ODM_WT.BSP.TP.FUNCTION.2018/12/04,Add skyw novatek TP IC
	...
	finger_cnt = 0;
	ts->nvt_oppo_proc_data->nvt_limit_area.which_area = NVT_AREA_DEFAULT;
...
//bin.su@ODM_WT.BSP.Tp,2019/08/12,solver probleml for RTC 2224750
			...
			ts->nvt_oppo_proc_data->nvt_point_info[i].x = (uint16_t) input_x;
			ts->nvt_oppo_proc_data->nvt_point_info[i].y = (uint16_t) input_y;
			ts->nvt_oppo_proc_data->nvt_point_info[i].type = NVT_AREA_NORMAL;
			if(ts->nvt_oppo_proc_data->edge_limit.limit_lu == 1 || ts->nvt_oppo_proc_data->edge_limit.limit_ru == 1 ||
				ts->nvt_oppo_proc_data->edge_limit.limit_lb == 1 || ts->nvt_oppo_proc_data->edge_limit.limit_rb == 1) {
...
//Bin.Su@ODM_WT.BSP.Tp.Init.1644722,2018/11/21,Bringup nt36525b+huaxian tp
	...
	if ( nt_ctpmodule == 0 ) {
		...
		NVT_LOG("not found hlt_boe nt36525b touchscreen\n");
		return -1;
	}
#endif
#if 0
...
//	}
	...
	//Bin.Su@ODM_WT.BSP.TP.FUNCTION.2019/08/16,adjust ABS_MT_POSITION_X,ABS_MT_POSITION_Y value
