...
//Bin.Su@ODM_WT.BSP.TP,2019/06/05,Add sign firmware function begain
	if (sign_firmware == 0) {
		...
		ipio_info("Open file method = %s, path = %s,sign_firmware = %d,ili_fw->oppo_sign_bin = %s\n",
			open_file_method ? "FILP_OPEN" : "REQUEST_FIRMWARE", open_file_method ? ili_fw->filp_open_bin_name:ili_fw->firmware_bin_name,sign_firmware,ili_fw->oppo_sign_bin);
	}
#endif
	...
	case REQUEST_FIRMWARE:
		if(idev->need_request_fw) {
			if(sign_firmware){
				if (request_firmware_select(&fw, ili_fw->oppo_sign_bin, idev->dev) < 0){
					ipio_err("Rquest ili_fw->oppo_sign_bin :%s firmware failed\n",ili_fw->oppo_sign_bin);
					return -ENOMEM;
				}
				ipio_info("ilitek_tdd_fw_hex_open ili_fw->oppo_sign_bin:%s success",ili_fw->oppo_sign_bin);
			}else{
