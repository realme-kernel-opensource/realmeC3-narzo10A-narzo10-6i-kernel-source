...
static int ilitek_tddi_mp_ini_parser(const char *path)
{
    ...
    ret = request_firmware(&fw_ini, path, &(idev->spi->dev));
    if (ret != 0) {
        ipio_err("%s : request mp file failed! ret = %d\n", __func__, ret);
		goto out;
    }

	ipio_info("ini file size = %ld\n", fw_ini->size);
	if (fw_ini->size <= 0) {
		ipio_err("The size of file is invaild\n");
		ret = -EINVAL;
		goto out;
	}
	tmp = vmalloc(fw_ini->size+1);
	if (ERR_ALLOC_MEM(tmp)) {
		ipio_err("Failed to allocate tmp memory, %ld\n", PTR_ERR(tmp));
		ret = -ENOMEM;
		goto out;
	}
	tmp[fw_ini->size] = 0x00;
	...
	for (i = 0; i < fw_ini->size; i++)
		tmp[i] = tolower(fw_ini->data[i]);

	ret = parser_get_ini_phy_data(tmp, fw_ini->size);
	if (ret < 0) {
...
static void mp_show_result(const char *csv_path)
{
	...
	if (f == NULL)
		f = filp_open(csv_name, O_RDWR | O_CREAT, 664);
	...
	if (ERR_ALLOC_MEM(f)) {
		...
		if (core_mp.final_result == MP_FAIL) {
			...
			sprintf(csv_name, "%s/%s_%s.csv", "/sdcard", get_date_time_str(), ret_pass_name);
		}
		f = filp_open(csv_name, O_RDWR | O_CREAT, 664);
		if (ERR_ALLOC_MEM(f)){
			ipio_err("Failed to open CSV file in sdcard");
			goto fail_open;
		}
		#endif
		goto fail_open;
...
int ilitek_tddi_mp_test_main(char *apk, bool lcm_on)
{
	...
	ret = ilitek_tddi_mp_ini_parser(ili_fw->mp_ini_name);
	if (ret < 0) {
