...
//Tongxing.Liu@ODM_WT.BSP.TP.Base,2019/10/05,Add sign firmware function begain
	if ((strncmp(cmd, "0", 1) == 0)||(strncmp(cmd, "1", 1) == 0)) {
		...
		ipio_info("ilitek_proc_oppo_upgrade_fw_write is not right\n");
		size = -1;
	}
#endif
	mutex_unlock(&idev->touch_mutex);
								  ...
								  size_t len, loff_t *pos)
{
  ...
  static ssize_t oppo_proc_tpcell_read(struct file *file, char *buf,
									size_t len, loff_t *pos)
  {
	  size_t ret = 0;
	  int res;
...
static ssize_t oppo_proc_hopping_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	int ret = 0;
	int len = 0;
	char *ptr = NULL;
...
static ssize_t oppo_proc_hopping_write(struct file *filp, const char *buff, size_t size, loff_t *pPos)
{
	char buf[8] = {0};
	int  temp;
	if ( *pPos ) {
...
struct file_operations proc_oppo_debug_hopping_fops = {
	.read  = oppo_proc_hopping_read,
	.write = oppo_proc_hopping_write,
};
...
int oppo_proc_init(void)
{
	...
	if ( proc_oppo_register_info == NULL )
	{
		ipio_err("create proc/touchpanel/oppo_register_info Failed!\n");
		res = -1;
	}
	proc_oppo_upgrade_fw_dir =proc_create("tp_fw_update", 0644, proc_dir_oppo, &proc_oppo_upgrade_fw_fops);
	if ( proc_oppo_upgrade_fw_dir == NULL )
	{
		ipio_err("create proc/touchpanel/tp_fw_update Failed!\n");
		res = -1;
	}
	proc_oppo_sign_firmware_dir =proc_create("sign_firmware", 0644, proc_dir_oppo, &proc_sign_firmware_fops);
	if ( proc_oppo_sign_firmware_dir == NULL )
	{
	...
	if ( proc_oppo_irq_depth_dir == NULL )
	{
	...
	if ( proc_oppo_i2c_device_test_dir == NULL )
	{
	...
	if ( proc_oppo_edge_limit_enable == NULL )
	{
		ipio_err("create proc/touchpanel/oppo_tp_limit_enable Failed!\n");
		res = -1;
	}
	proc_oppo_tp_direction_dir = proc_create("oppo_tp_direction", 0666, proc_dir_oppo, &ilitek_direction_fops);
	if ( proc_oppo_tp_direction_dir == NULL )
	{
		ipio_err("create proc/touchpanel/oppo_tp_direction Failed!\n");
		res = -1;
	}
	proc_oppo_tpcell_info =proc_create("incell_panel", 0666, proc_dir_oppo, &proc_tpcell_fops);
	if ( proc_oppo_i2c_device_test_dir == NULL )
	{
	...
	proc_hopping_node =proc_create("freq_hop_simulate", 0666, proc_dir_debug_info, &proc_oppo_debug_hopping_fops);
