...
bool hx_parse_bin_cfg_data(const struct firmware *fw_entry)
{
    ...
    for (i = 0; i < 4; i++)
        hx83112a_nf_sram_min[i] = zf_info_arr[i_min].sram_addr[i];
    hx83112a_nf_cfg_sz = (dsram_max - dsram_base) + zf_info_arr[i_max].write_size;
    ...
    for (i = 1; i < part_num; i++)
        memcpy(hx83112a_nf_FW_buf + (zf_info_arr[i].cfg_addr - dsram_base), (unsigned char *)&fw_entry->data[zf_info_arr[i].fw_addr], zf_info_arr[i].write_size);

    hx83112a_nf_cfg_crc = himax_mcu_Calculate_CRC_with_AP(hx83112a_nf_FW_buf, 0, hx83112a_nf_cfg_sz);
...
static int hx83112b_nf_zf_part_info(const struct firmware *fw_entry)
{
    ...
    do {
        himax_mcu_register_write(hx83112a_nf_sram_min, hx83112a_nf_cfg_sz, hx83112a_nf_FW_buf, 0);
        /*himax_register_write(hx83112a_nf_sram_min, hx83112a_nf_cfg_sz, hx83112a_nf_FW_buf, 0); */
        crc = himax_hw_check_CRC(hx83112a_nf_sram_min, hx83112a_nf_cfg_sz);
...
void himax_read_OPPO_FW_ver(struct chip_data_hx83112b *chip_info)
{
    uint8_t cmd[4];
    uint8_t data[4];
...
int hx_test_data_pop_out(struct chip_data_hx83112b *chip_info, char *g_Test_list_log, char *g_Company_info_log, char *g_project_test_info_log, char *rslt_buf, char *filepath)
{
...
int hx_test_data_get(struct chip_data_hx83112b *chip_info, uint32_t RAW[], char *start_log, char *result, int now_item)
{
    uint32_t i;
...
int hx_print_crtra_after_parsing(struct chip_data_hx83112b *chip_info)
{
    int i = 0, j = 0;
...
static int hx_get_crtra_by_name(struct chip_data_hx83112b *chip_info, char **result, int size_of_result_str)
{
    int i = 0;
...
static int himax_parse_criteria_file(struct chip_data_hx83112b *chip_info)
{
    int err = THP_AFE_INSPECT_OK;
...
static int himax_self_test_data_init(struct chip_data_hx83112b *chip_info)
{
    ...
    /* init criteria data*/
    hx83112a_nf_inspt_crtra_flag = kzalloc(HX_CRITERIA_SIZE * sizeof(int), GFP_KERNEL);
    hx83112a_nf_inspection_criteria = kzalloc(sizeof(int *)*HX_CRITERIA_SIZE, GFP_KERNEL);
...
int himax_chip_self_test(struct seq_file *s, struct chip_data_hx83112b *chip_info, char *g_Test_list_log)
{
    int error = 0;
    int error_num = 0;
    char buf[128] = {0};
...
static void himax_debug_sta_judge(struct chip_data_hx83112b *chip_info)
{
...
*/
static int hx83112b_int_pin_test(struct seq_file *s, void *chip_data, struct syna_testdata *syna_testdata, char *g_Test_list_log)
{
...
static void hx83112b_auto_test(struct seq_file *s, void *chip_data, struct syna_testdata *syna_testdata)
{
    ...
    error_count += hx83112b_int_pin_test(s, chip_info, syna_testdata, g_Test_list_log);
...
static void hx83112b_delta_read(struct seq_file *s, void *chip_data)
{
    ...
    chip_info = (struct chip_data_hx83112b *)chip_data;
    hx83112b_read_debug_data(s, chip_data, DEBUG_DATA_DELTA);
#ifdef CONFIG_OPPO_TP_APK
    if (chip_info->debug_mode_sta) {
        hx83112b_read_debug_data(s, chip_data, DEBUG_DATA_DOWN);
    }
#endif // end of CONFIG_OPPO_TP_APK
    return;
}
...
int hx83112b_freq_point = 0;
void hx83112b_freq_hop_trigger(void *chip_data)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    int rtimes = 0;
    ...
    hx83112b_freq_point = 1 - hx83112b_freq_point;
    if (hx83112b_freq_point) {//hop to frequency 130K
        do {
            if (rtimes > 10) {
...
static struct oppo_touchpanel_operations hx83112b_ops = {
    ...
    .freq_hop_trigger = hx83112b_freq_hop_trigger,
};
...
{
    hx83112b_mode_switch(chip_data, MODE_GAME, on_off);
}
...
static void himax_apk_earphone_set(void *chip_data, bool on_off)
{
    struct chip_data_hx83112b *chip_info;
    chip_info = (struct chip_data_hx83112b *)chip_data;
    hx83112b_mode_switch(chip_data, MODE_HEADSET, on_off);
...
static void himax_apk_charger_set(void *chip_data, bool on_off)
{
    struct chip_data_hx83112b *chip_info;
    chip_info = (struct chip_data_hx83112b *)chip_data;
    hx83112b_mode_switch(chip_data, MODE_CHARGE, on_off);
