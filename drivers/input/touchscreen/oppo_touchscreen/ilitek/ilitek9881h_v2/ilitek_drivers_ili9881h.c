/***************************************************
 * File:ilitek_drivers_ili9881h.c
 * VENDOR_EDIT
...
static int parse_touch_package_v5_0(uint8_t pid)
{
    ...
    if (pid == protocol->demo_pid) {
        ...
        if (ipd->oppo_read_debug_data && (!ERR_ALLOC_MEM(ipd->oppo_debug_buf))) {
            for (index = 0, i = 35; i < count + 35; i+=2, index++) {
                if((uint8_t)(g_fr_node->data[i] & 0x80) == (uint8_t)0x80)
                {
                    ...
                    ipd->oppo_debug_buf[index] = (((g_fr_node->data[i] << 8) + g_fr_node->data[i+1]) - 0x10000);
                }
                else
                {
                    TPD_DEBUG("%d, ", (g_fr_node->data[i] << 8) + g_fr_node->data[i+1]);
                    ipd->oppo_debug_buf[index] = ((g_fr_node->data[i] << 8) + g_fr_node->data[i+1]);
                }
            }
            ipd->oppo_read_debug_data = false;
        }
    } else {
        if (pid != 0) {
...
static int ilitek_read_debug_data(struct seq_file *s, struct ilitek_chip_data_9881h *chip_info, DEBUG_READ_TYPE read_type)
{
    ...
    chip_info->oppo_debug_buf = (int *)kzalloc((xch * ych) * sizeof(int), GFP_KERNEL);
    if (ERR_ALLOC_MEM(chip_info->oppo_debug_buf)) {
        TPD_INFO("Failed to allocate oppo_debug_buf memory, %ld\n", PTR_ERR(chip_info->oppo_debug_buf));
        return -ENOMEM;
    }
    ...
    enable_irq(ipd->isr_gpio);//because oppo disable
    chip_info->oppo_read_debug_data = true;
    for (i = 0; i < 1000; i++) {
        msleep(5);
        if (!chip_info->oppo_read_debug_data) {
    ...
    for (i = 0; i < ych; i++) {
        seq_printf(s, "[%2d]", i);
        for (j = 0; j < xch; j++) {
            seq_printf(s, "%5d, ", chip_info->oppo_debug_buf[i * xch + j]);
        }
        seq_printf(s, "\n");
    }
    ...
    core_config_mode_control(test_cmd);
    mutex_lock(&ipd->plat_mutex);
    ipio_kfree((void **)&chip_info->oppo_debug_buf);
    mutex_unlock(&ipd->plat_mutex);
    return 0;
}
