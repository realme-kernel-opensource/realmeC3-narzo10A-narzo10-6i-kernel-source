...
int ssusb_check_clocks(struct ssusb_mtk *ssusb, u32 ex_clks)
{
	...
	if (ret)
		dev_info(ssusb->dev, "clks of sts1 are not stable!\n");

	ret = readl_poll_timeout(ibase + U3D_SSUSB_IP_PW_STS2, value,
			(value & SSUSB_U2_MAC_SYS_RST_B_STS), 100, 10000);
	if (ret)
		dev_info(ssusb->dev, "mac2 clock is not stable\n");
...
static int ssusb_rscs_init(struct ssusb_mtk *ssusb)
{
	...
	if (ret) {
		dev_info(ssusb->dev, "failed to init phy\n");
		goto phy_init_err;
	}
...
static int get_ssusb_rscs(struct platform_device *pdev, struct ssusb_mtk *ssusb)
{
	...
	for (i = 0; i < ssusb->num_phys; i++) {
		...
		if (IS_ERR(ssusb->phys[i])) {
			dev_info(dev, "failed to get phy-%d\n", i);
	...
	if (of_property_read_bool(node, "extcon")) {
		...
		if (IS_ERR(otg_sx->edev)) {
			dev_info(ssusb->dev, "couldn't get extcon device\n");
			return -EPROBE_DEFER;
		}
	}
