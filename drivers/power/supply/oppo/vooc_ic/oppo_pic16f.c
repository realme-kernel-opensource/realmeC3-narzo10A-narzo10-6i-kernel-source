...
static int oppo_vooc_i2c_write(struct i2c_client *client, u8 addr, s32 len, u8 *txbuf)
{
#ifdef CONFIG_OPPO_CHARGER_MTK
...
static bool pic16f_fw_check(struct oppo_vooc_chip *chip)
{
...
static void pic16f_fw_data_recover(struct oppo_vooc_chip *chip,unsigned char *data_buf,unsigned int offset,unsigned int length)
{
...
static int pic16f_fw_write(struct oppo_vooc_chip *chip,unsigned char *data_buf,unsigned int offset,unsigned int length)
{
	unsigned int count = 0;
...
static int pic16f_fw_update(struct oppo_vooc_chip *chip)
...
static void pic16f_fw_check_then_recover(struct oppo_vooc_chip *chip)
{
	int update_result = 0;

	chg_debug( " begin\n");
	if(oppo_is_power_off_charging(chip) == true) {
		chip->mcu_update_ing = true;
...
struct oppo_vooc_operations oppo_pic16f_ops = {
...
{             
	struct oppo_vooc_chip *chip = NULL;
...
MODULE_DESCRIPTION("Driver for oppo vooc pic16f fast mcu");
