		...
		for (j = 0; j < 16; j++) {
			if (data_buf[j] != chip->firmware_data[fw_line * 18 + 2 + j]) {
				chg_err("fail, data_buf[%d]:0x%x != n76e_firmware_data[%d]:0x%x\n",
						j, data_buf[j], (fw_line * 18 + 2 + j), chip->firmware_data[fw_line * 18 + 2 + j]);
				chg_err("addr = 0x%x, %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x\n",
						((addr_buf[0] << 8) | addr_buf[1]), data_buf[0], data_buf[1], data_buf[2], data_buf[3],
						data_buf[4], data_buf[5], data_buf[6], data_buf[7],data_buf[8], data_buf[9], data_buf[10],
...
static int n76e_fw_write(struct oppo_vooc_chip *chip, const unsigned char *data_buf, unsigned int offset, unsigned int length)
{
