...
extern	int oppo_vooc_mcu_hwid_check(struct oppo_vooc_chip *chip);
int get_vooc_mcu_type(struct oppo_vooc_chip *chip)
{
        if(chip == NULL){
                chg_err("oppo_vooc_chip is not ready, enable stm8s\n");
                return OPPO_VOOC_MCU_HWID_STM8S;
        }
        return oppo_vooc_mcu_hwid_check(chip);
}
...
/*This is only for P60 P(17197 P)*/
        ...
        if (IS_ERR_OR_NULL(chip->vooc_gpio.gpio_vooc_mcu_id_default)) {
                chg_err(": %d Failed to get the gpio_vooc_mcu_id_default pinctrl handle, enable stm8s\n", __LINE__);
				mcu_hwid_type = OPPO_VOOC_MCU_HWID_STM8S;
                return OPPO_VOOC_MCU_HWID_STM8S;
        }
        ...
        if (gpio_is_valid(chip->vooc_gpio.vooc_mcu_id_gpio)){
                if(gpio_get_value(chip->vooc_gpio.vooc_mcu_id_gpio) == 0){
						...
						mcu_hwid_type = OPPO_VOOC_MCU_HWID_N76E;
...
int  oppo_is_charging_now(void)
{
       return  oppo_chg_get_charger_exist();
}
