...
#define CONFIG_OPPO_CHARGER_QCOM
...
static int smbchg_read(struct oppo_chg_chip *chip, u8 *val,
			u16 addr, int count)
{
	int rc = 0;
 ...
 */
static int smbchg_write(struct oppo_chg_chip *chip, u8 *val,
			u16 addr, int count)
{
	int rc = 0;
	struct spmi_device *spmi = chip->pmic_spmi.spmi;
 ...
 * random secure access writes may fail.
 */
static int smbchg_masked_write_raw(struct oppo_chg_chip *chip, u16 base, u8 mask,
									u8 val)
{
	int rc;
	u8 reg;
 ...
 */
static int smbchg_masked_write(struct oppo_chg_chip *chip, u16 base, u8 mask,
								u8 val)
{
	unsigned long flags;
	int rc;
...
static int smbchg_sec_masked_write(struct oppo_chg_chip *chip, u16 base, u8 mask,
									u8 val)
{
	unsigned long flags;
	int rc;
...
static void smbchg_stay_awake(struct oppo_chg_chip *chip, int reason)
{
	int reasons;
...
static void smbchg_relax(struct oppo_chg_chip *chip, int reason)
{
	int reasons;
...
static enum pwr_path_type smbchg_get_pwr_path(struct oppo_chg_chip *chip)
{
	int rc;
	u8 reg;
...
static bool is_otg_present_schg(struct oppo_chg_chip *chip)
{
	int rc;
	u8 reg;
	u8 usbid_reg[2];
...
static bool is_otg_present_schg_lite(struct oppo_chg_chip *chip)
{
	int rc;
	u8 reg;
...
static bool is_otg_present(struct oppo_chg_chip *chip)
{
...
static bool is_dc_present(struct oppo_chg_chip *chip)
{
	int rc;
	u8 reg;
...
static bool is_usb_present(struct oppo_chg_chip *chip)
{
	int rc;
	u8 reg;
...
static int oppo_set_primal_type(struct power_supply *psy,int primal_type)
{
...
static void read_usb_type(struct oppo_chg_chip *chip, char **usb_type_name,
...
int get_prop_batt_status(struct oppo_chg_chip *chip)
{
...
int smbchg_read_full(struct oppo_chg_chip *chip)
{
	int rc,status = 0;
	u8 reg = 0;
...
static int get_prop_batt_present(struct oppo_chg_chip *chip)
{
	int rc;
	u8 reg;
...
int get_prop_charge_type(struct oppo_chg_chip *chip)
{
	int rc;
	u8 reg, chg_type;
...
static int set_property_on_fg(struct oppo_chg_chip *chip,
...
static int get_property_from_fg(struct oppo_chg_chip *chip,
		enum power_supply_property prop, int *val)
{
	int rc;
	union power_supply_propval ret = {0, };
...
static int get_prop_batt_capacity(struct oppo_chg_chip *chip)
{
	int capacity, rc;
...
static int get_prop_batt_temp(struct oppo_chg_chip *chip)
{
	int temp, rc;
...
static int get_prop_batt_current_now(struct oppo_chg_chip *chip)
{
	int ua, rc;
...
static int get_prop_batt_voltage_now(struct oppo_chg_chip *chip)
{
	int uv, rc;
...
static int get_prop_batt_voltage_max_design(struct oppo_chg_chip *chip)
{
	int uv, rc;
...
#if 1 //use the oppo_chg_get_prop_batt_health instead;
static int get_prop_batt_health(struct oppo_chg_chip *chip)
{
...
static int calc_thermal_limited_current(struct oppo_chg_chip *chip,
						int current_ma)
{
	int therm_ma;
...
static int smbchg_charging_en(struct oppo_chg_chip *chip, bool en)
{
...
static int smbchg_dc_suspend(struct oppo_chg_chip *chip, bool suspend)
{
	int rc = 0;
...
static int smbchg_set_dc_current_max(struct oppo_chg_chip *chip, int current_ma)
{
	int i;
	u8 dc_cur_val;
...
static struct power_supply *get_parallel_psy(struct oppo_chg_chip *chip)
{
...
static void smbchg_usb_update_online_work(struct work_struct *work)
{
	struct qcom_pmic *pmic_chip = container_of(work,
				struct qcom_pmic,
				usb_set_online_work);
	struct oppo_chg_chip *chip = container_of(pmic_chip,
...
static bool smbchg_primary_usb_is_en(struct oppo_chg_chip *chip,
...
#if 0 //delected by PengNan 12.13
static bool smcghg_is_battchg_en(struct oppo_chg_chip *chip,
...
#endif
static int smbchg_battchg_en(struct oppo_chg_chip *chip, bool enable,
...
static int smbchg_primary_usb_en(struct oppo_chg_chip *chip, bool enable,
...
static int smbchg_dc_en(struct oppo_chg_chip *chip, bool enable,
		enum enable_reason reason)
{
	int rc = 0, suspended;
...
#if 0
static int smbchg_set_high_usb_chg_current(struct oppo_chg_chip *chip,
							int current_ma)
{
	int i, rc;
	u8 usb_cur_val;
...
//oppo_usbin_current_ma_table[] must match with oppo_usbin_current_regval_table[]
static const int oppo_usbin_current_ma_table[] = {
	500,
	950,//900->950
	1200,
	1500,
...
static const int oppo_usbin_current_regval_table[] = {
	0x4,		//500ma,
	0xa,		//900ma->950
	0xd,		//1200ma,
	0x10,		//1500ma,
...
static int find_usb_current_in_oppo_array
			(const int *table, int val)
{
	int i;
	int len = ARRAY_SIZE(oppo_usbin_current_ma_table);
...
static void smbchg_write_usb_input_current_reg
				(struct oppo_chg_chip *chip, int reg_ma)
{
...
static bool smbchg_check_soft_aicl_pass_ornot(struct oppo_chg_chip *chip, 
								int reg_ma)
{
	int chg_vol = 0;
...
extern int oppo_gauge_get_batt_temperature(void);		// add for avoid aicl when temp < 5 degree
static int smbchg_set_high_usb_chg_current(struct oppo_chg_chip *chip,
							int current_ma)
{
	int rc = 0, i, icl_ma, aicl_max_ma;
	bool aicl_pass = false;
	int temp = oppo_gauge_get_batt_temperature();
	...
	if(chip->pmic_spmi.not_support_1200ma) {
		if(temp <= 50) {
			...
			smbchg_write_usb_input_current_reg(chip, oppo_usbin_current_regval_table[1]);
			smbchg_rerun_aicl(chip);
			chip->pmic_spmi.usb_max_current_ma = oppo_usbin_current_ma_table[1];
			chg_err("Low Temp,No aicl and Input_Current_limit = %d (0x%02x), rc = %d\n",
				chip->pmic_spmi.usb_max_current_ma,oppo_usbin_current_regval_table[1],rc);
			return rc;
		}
	}

	icl_ma = find_usb_current_in_oppo_array(oppo_usbin_current_ma_table, current_ma);
	...
	for(i = 0; i <= aicl_max_ma; i++) {
		aicl_pass = smbchg_check_soft_aicl_pass_ornot(chip, oppo_usbin_current_regval_table[i]);
		if(aicl_pass == false) {
			if(i > 0){
				i--;
			}
			smbchg_write_usb_input_current_reg(chip, oppo_usbin_current_regval_table[i]);
			break;
		}
	}
	if(i == 2) {	// skip 1200ma
	...
	smbchg_rerun_aicl(chip);
	chip->pmic_spmi.usb_max_current_ma = oppo_usbin_current_ma_table[i];
	chg_err("current_ma = %d, reg_ma = 0x%x, max_current_ma = %d\n",
		current_ma, oppo_usbin_current_regval_table[i], chip->pmic_spmi.usb_max_current_ma);
	return rc;
}

#endif
int qpnp_charger_type_get(struct oppo_chg_chip *chip) 
{ 
    union power_supply_propval ret = {0,}; 
 ...
 */
static int smbchg_set_usb_current_max(struct oppo_chg_chip *chip,
							int current_ma)
{
	int rc = 0;
	bool changed;
...
static int smbchg_get_min_parallel_current_ma(struct oppo_chg_chip *chip)
{
	int rc;
...
/*there return false*/
static bool smbchg_is_parallel_usb_ok(struct oppo_chg_chip *chip)
{
	...
	if (oppo_chg_get_prop_batt_health(chip) != POWER_SUPPLY_HEALTH_GOOD) {
...
static int smbchg_set_fastchg_current(struct oppo_chg_chip *chip,
							int current_ma)
{
	int rc = 0;
...
/*Here return 0*/
#if 1
static int smbchg_parallel_usb_charging_en(struct oppo_chg_chip *chip, bool en)
{
...
#if 1
static int smbchg_sw_esr_pulse_en(struct oppo_chg_chip *chip, bool en)
{
	int rc;
...
void smbchg_rerun_aicl(struct oppo_chg_chip *chip)
{
...
static void taper_irq_en(struct oppo_chg_chip *chip, bool en)
{
...
/*return null*/
static void smbchg_parallel_usb_disable(struct oppo_chg_chip *chip)
{
	struct power_supply *parallel_psy = get_parallel_psy(chip);
...
/*return null*/
static void smbchg_parallel_usb_taper(struct oppo_chg_chip *chip)
{
	struct power_supply *parallel_psy = get_parallel_psy(chip);
	union power_supply_propval pval = {0, };
...
static bool smbchg_is_aicl_complete(struct oppo_chg_chip *chip)
{
	int rc;
	u8 reg;
...
#endif
int smbchg_get_aicl_level_ma(struct oppo_chg_chip *chip)
{
	int rc;
	u8 reg;
...
/*return null*/
static void smbchg_parallel_usb_enable(struct oppo_chg_chip *chip)
{
	struct power_supply *parallel_psy = get_parallel_psy(chip);
	union power_supply_propval pval = {0, };
...
/*return null*/
static void smbchg_parallel_usb_check_ok(struct oppo_chg_chip *chip)
{
	struct power_supply *parallel_psy = get_parallel_psy(chip);
...
static int smbchg_usb_en(struct oppo_chg_chip *chip, bool enable,
		enum enable_reason reason)
{
	bool changed = false;
...
static int smbchg_set_fastchg_current_user(struct oppo_chg_chip *chip,
							int current_ma)
{
	int rc = 0;
...
static struct ilim_entry *smbchg_wipower_find_entry(struct oppo_chg_chip *chip,
...
static int smbchg_dcin_ilim_config(struct oppo_chg_chip *chip, int offset, int ma)
{
	int i, rc;
...
static int smbchg_wipower_ilim_config(struct oppo_chg_chip *chip,
...
static int smbchg_wipower_dcin_btm_configure(struct oppo_chg_chip *chip,
		struct ilim_entry *ilim)
{
	int rc;
...
static int smbchg_wipower_icl_configure(struct oppo_chg_chip *chip,
...
static void smbchg_wipower_icl_deconfigure(struct oppo_chg_chip *chip)
{
	int rc;
...
static void __smbchg_wipower_check(struct oppo_chg_chip *chip)
{
	int chg_type;
...
static void smbchg_wipower_check(struct oppo_chg_chip *chip)
{
...
int qpnp_get_prop_charger_voltage_now_raw(struct oppo_chg_chip *chip)
{
 ...
 */
static int smbchg_set_thermal_limited_dc_current_max(struct oppo_chg_chip *chip,
							int current_ma)
{
 ...
 * that may be limited by the system's thermal level
 */
static int smbchg_set_thermal_limited_usb_current_max(struct oppo_chg_chip *chip,
							int current_ma)
{
	int rc, aicl_ma;
...
int smbchg_system_temp_level_set(struct oppo_chg_chip *chip,
								int lvl_sel)
{
	int rc = 0;
	int prev_therm_lvl;
...
int smbchg_calc_max_flash_current(struct oppo_chg_chip *chip)
{
...
static int smbchg_fastchg_current_comp_set(struct oppo_chg_chip *chip,
					int comp_current)
{
	int rc;
	u8 i;
...
static int smbchg_float_voltage_comp_set(struct oppo_chg_chip *chip, int code)
{
	int rc;
	u8 val;
...
int smbchg_float_voltage_set(struct oppo_chg_chip *chip, int vfloat_mv)
{
	struct power_supply *parallel_psy = get_parallel_psy(chip);
	int rc, delta;
	u8 temp;
...
#if 0 //deleted by PengNan 12.13
static int smbchg_float_voltage_get(struct oppo_chg_chip *chip)
{
...
int smbchg_safety_timer_enable(struct oppo_chg_chip *chip, bool enable)
{
	int rc;
	u8 reg;
...
int smbchg_otg_pulse_skip_disable(struct oppo_chg_chip *chip,
...
int smbchg_force_tlim_en(struct oppo_chg_chip *chip, bool enable)
{
	int rc;
...
static void smbchg_vfloat_adjust_check(struct oppo_chg_chip *chip)
{
...
static bool smbchg_is_input_current_limited(struct oppo_chg_chip *chip)
{
	int rc;
	u8 reg;
...
#if 1
static void smbchg_cc_esr_wa_check(struct oppo_chg_chip *chip)
{
	int rc, esr_count;
...
#endif
#if 1
static void smbchg_soc_changed(struct oppo_chg_chip *chip)
{
...
static int smbchg_hw_aicl_rerun_en(struct oppo_chg_chip *chip, bool en)
{
	int rc = 0;
...
static int smbchg_aicl_config(struct oppo_chg_chip *chip)
{
	int rc = 0;
...
static void smbchg_aicl_deglitch_wa_en(struct oppo_chg_chip *chip, bool en)
{
	int rc;
...
* here return 
*/
static void smbchg_aicl_deglitch_wa_check(struct oppo_chg_chip *chip)
{
...
static int smbchg_config_chg_battery_type(struct oppo_chg_chip *chip)
{
...
static void check_battery_type(struct oppo_chg_chip *chip)/////////pppp
{
	union power_supply_propval prop = {0,};
	bool en;
	bool unused;
...
static void smbchg_set_chargerid_switch_val(
				struct oppo_chg_chip *chip, int value)
{
	if(chip->normalchg_gpio.chargerid_switch_gpio <= 0) {
...
void oppo_chg_external_power_changed(struct power_supply *psy)/////pppp
{
	struct oppo_chg_chip *chip = container_of(psy,
				struct oppo_chg_chip, batt_psy);
	union power_supply_propval prop = {0,};
...
{
	int rc = 0;
	struct oppo_chg_chip *chip = rdev_get_drvdata(rdev);
...
static int smbchg_regulator_init(struct oppo_chg_chip *chip)
{
	int rc = 0;
...
static void smbchg_regulator_deinit(struct oppo_chg_chip *chip)
{
...
static int smbchg_chg_led_controls(struct oppo_chg_chip *chip)
{
	u8 reg, mask;
	int rc;
...
static void smbchg_chg_led_blink_set(struct oppo_chg_chip *chip,//////plant  to the main function next times
...
static int smbchg_register_chg_led(struct oppo_chg_chip *chip)
{
	int rc;
...
static int smbchg_adjust_vfloat_mv_trim(struct oppo_chg_chip *chip,
						int delta_mv)
{
...
static int smbchg_charging_status_change(struct oppo_chg_chip *chip)
{
	smbchg_vfloat_adjust_check(chip);
...
static bool is_hvdcp_present(struct  oppo_chg_chip *chip)
{
	int rc;
	u8 reg, hvdcp_sel;
...
static int opchg_hvdcp_enable(struct oppo_chg_chip *chip,bool enable)
{
	int rc;
...
static int force_9v_hvdcp(struct oppo_chg_chip *chip)
{
	int rc;
...
static int set_usb_psy_dp_dm(struct oppo_chg_chip *chip, int state)
{
	int rc;
	u8 reg;
...
static void restore_from_hvdcp_detection(struct oppo_chg_chip *chip)
{
	int rc;
	return;//do nothing
	/* switch to 9V HVDCP */
...
*/
static void handle_usb_removal(struct oppo_chg_chip *chip)
{
	struct power_supply *parallel_psy = get_parallel_psy(chip);
	int rc;
...
static bool is_src_detect_high(struct oppo_chg_chip *chip)
{
	int rc;
	u8 reg;
...
static bool is_usbin_uv_high(struct oppo_chg_chip *chip)
{
	int rc;
	u8 reg;
...
*/
static void handle_usb_insertion(struct oppo_chg_chip *chip)
{
	struct power_supply *parallel_psy = get_parallel_psy(chip);
	enum power_supply_type usb_supply_type;
	int rc;
...
//	chip->charger_type = usb_supply_type;
	...
	if (chip->usb_psy) {
		...
		oppo_set_primal_type(chip->usb_psy,chip->pmic_spmi.pmic_type);
		pr_smb(PR_MISC, "setting usb psy present = %d\n",
				chip->pmic_spmi.usb_present);
		power_supply_set_present(chip->usb_psy, chip->pmic_spmi.usb_present);
		/* Notify the USB psy if OV condition is not present */
...
void update_usb_status(struct oppo_chg_chip *chip, bool usb_present, bool force)
{
...
static int otg_oc_reset(struct oppo_chg_chip *chip)
{
	int rc;
...
#endif
/*tell that */
static void increment_aicl_count(struct oppo_chg_chip *chip)////pppp
{
...
static int wait_for_usbin_uv(struct oppo_chg_chip *chip, bool high)
{
	int rc;
	int tries = 3;
...
static int wait_for_src_detect(struct oppo_chg_chip *chip, bool high)
{
	int rc;
	int tries = 3;
...
static int fake_insertion_removal(struct oppo_chg_chip *chip, bool insertion)
{
	int rc;
	bool src_detect;
	bool usbin_uv;
...
static int smbchg_prepare_for_pulsing(struct oppo_chg_chip *chip)
{
	int rc = 0;
	u8 reg;
...
static int smbchg_unprepare_for_pulsing(struct oppo_chg_chip *chip)
{
	int rc = 0;
...
static int rerun_apsd(struct oppo_chg_chip *chip)
{
	int rc;
...
static bool is_hvdcp_5v_cont_mode(struct oppo_chg_chip *chip)
{
	int rc;
	u8 reg = 0;
...
static int smbchg_prepare_for_pulsing_lite(struct oppo_chg_chip *chip)
{
	int rc = 0;
...
static int smbchg_unprepare_for_pulsing_lite(struct oppo_chg_chip *chip)
{
	int rc = 0;
...
static int smbchg_dp_pulse_lite(struct oppo_chg_chip *chip)
{
	int rc = 0;
...
static int smbchg_dm_pulse_lite(struct oppo_chg_chip *chip)
{
	int rc = 0;
...
static int smbchg_hvdcp3_confirmed(struct oppo_chg_chip *chip)
{
	int rc;
...
int smbchg_dp_dm(struct oppo_chg_chip *chip, int val)
{
	int rc = 0;
...
/**/
	...
	case POWER_SUPPLY_PROP_BATTERY_REQUEST_POWEROFF:
		...
		case POWER_SUPPLY_PROP_FAST_CHARGE:
			val->intval = oppo_vooc_get_fastchg_started();
...
{
	struct oppo_chg_chip *chip = _chip;
	u8 reg = 0;
 ...
 * @chip: pointer to oppo_chg_chip chip
 */
 /*the function*
*update usb state
*/
...
static int determine_initial_status(struct oppo_chg_chip *chip)
{
	/*
...
*
*/
static int smbchg_hw_init(struct oppo_chg_chip *chip)
{
	int rc, i;
	u8 reg, mask;
...
static int smb_parse_wipower_map_dt(struct oppo_chg_chip *chip,
...
static int smb_parse_wipower_dt(struct oppo_chg_chip *chip)
{
	int rc = 0;
...
static int smb_parse_dt(struct oppo_chg_chip *chip)
{
...
static int smbchg_request_irqs(struct oppo_chg_chip *chip)
{
	int rc = 0;
...
static int smbchg_parse_peripherals(struct oppo_chg_chip *chip)
{
	int rc = 0;
	struct resource *resource;
	struct spmi_resource *spmi_resource;
...
static inline void dump_reg(struct oppo_chg_chip *chip, u16 addr,
		const char *name)
{
	u8 reg;
...
{
	struct oppo_chg_chip *chip = container_of(work,
				struct oppo_chg_chip,
				dump_reg_work.work);
...
#endif
static int create_debugfs_entries(struct oppo_chg_chip *chip)
{
	struct dentry *ent;
...
static int smbchg_wa_config(struct oppo_chg_chip *chip)
{
...
static int smbchg_check_chg_version(struct oppo_chg_chip *chip)
{
	int rc;
	u8 val = 0;
...
int opchg_get_prop_charge_type(struct oppo_chg_chip *chip)
{
	int rc;
...
int opchg_get_prop_batt_status(struct oppo_chg_chip *chip)
{
...
static int opchg_get_charger_type(struct oppo_chg_chip *chip)
{
...
void opchg_check_status(struct oppo_chg_chip *chip)
{
	chg_err(" begin----------\n");
...
static int opchg_check_charging_full(struct oppo_chg_chip *chip)
{	
	return 0;
}
int opchg_set_input_chg_current(struct oppo_chg_chip *chip)
{
	int rc = 0;
...
int opchg_set_fast_chg_current(struct oppo_chg_chip *chip, int mA)
{
	int rc = 0;
	smbchg_set_fastchg_current(chip,mA);
	return rc;
}
int opchg_set_float_voltage(struct oppo_chg_chip *chip, int mV)
{	
	int rc = 0;
	smbchg_float_voltage_set(chip,mV);
	return rc;
}
int opchg_set_charging_disable(struct oppo_chg_chip *chip, bool disable)
{
	int rc = 0;
...
int opchg_set_suspend_enable(struct oppo_chg_chip *chip, bool enable)
{
	int rc = 0;
...
int opchg_read_full(struct oppo_chg_chip *chip)
{
	return 0;
}
...
void opchg_set_status(struct oppo_chg_chip *chip)
{
...
#endif
static void opchg_works_init(struct oppo_chg_chip *chip)
{
...
void otg_enable_id_value (void)
{
	...
	u8 reg = 0;
	if(!the_chip) {
		pr_err("[OPPO_CHG][%s]: smb_chg not ready!\n",__func__);
		return;
	}
	...
	if(rc < 0)
		pr_err("[OPPO_CHG][%s]:  Can't enable otg_id value, rc = %d\n",__func__,rc);
	else
		pr_err("[OPPO_CHG][%s]:  RC = %d\n",__func__,rc);

	rc = smbchg_read(the_chip, &reg, the_chip->pmic_spmi.otg_base + OTG_CFG, 1);
	if (rc < 0) {
		pr_err("[OPPO_CHG][%s]:Unable to read OTG_CFG rc = %d\n",__func__,rc);
	} else {
		pr_err("[OPPO_CHG][%s]:  Reg = 0x%02x\n",__func__,reg);
	}
}
...
void otg_disable_id_value (void)
{
	...
	rc = smbchg_sec_masked_write(the_chip, the_chip->pmic_spmi.otg_base + OTG_CFG,
			OTG_CTRL_MASK, OTG_RID_DIS);
	if(rc < 0)
		pr_err("[OPPO_CHG][%s]:  Can't disable otg_id value, rc = %d\n",__func__,rc);
	else
		pr_err("[OPPO_CHG][%s]:  RC = %d\n",__func__,rc);
...
static int smbchg_set_chging_term_disable(struct oppo_chg_chip *chip)
{
	return 0;
}
...
static struct oppo_chg_operations  smb_chg_ops = {
...
{
	struct oppo_chg_chip *chip = dev_get_drvdata(&spmi->dev);
