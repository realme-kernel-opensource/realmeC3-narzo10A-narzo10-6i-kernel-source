...
static bool oppochg_usbtemp_condition(void)
{
	int rc;
	bool vbus_rising;
	u8 stat;
...
static void oppochg_usbtemp_dischg(void)
{
	int rc;
	struct smb_charger *chg = NULL;
	...
	usleep_range(10000,11000);
	g_oppo_chip->chg_ops->charger_suspend();
	usleep_range(10000,11000);

	pinctrl_select_state(g_oppo_chip->normalchg_gpio.pinctrl, g_oppo_chip->normalchg_gpio.dischg_enable);
}
...
static int oppochg_usbtemp_monitor_main(void *data)
{
	...
	detect_interval = DETECT_INTERVAL_300MS;

	while (!kthread_should_stop()) {
		if (oppochg_usbtemp_condition() == true) {
			voltage1 = 0;
			voltage2 = 0;
			for (index = 0; index < ADC_SAMPLE_COUNT; index ++) {
				voltage1 += oppochg_read_adc(USB_TEMPERATURE1);	/* PM7150A ADC */
				voltage2 += oppochg_read_adc(USB_TEMPERATURE2);	/* PM7150 ADC */
			...
			g_oppo_chip->usbtemp_volt_l = voltage1;
			g_oppo_chip->usbtemp_volt_r = voltage2;
			...
			if (oppo_vooc_get_fastchg_started() == true)
				bat_temp = oppo_gauge_get_prev_batt_temperature();
			else
				bat_temp = oppo_gauge_get_batt_temperature();
			...
			if (dischg_enable == true) {
				oppochg_usbtemp_dischg();
				break;
			}
		} else {
			do_gettimeofday(&time1);
			wait_event_interruptible(oppochg_usbtemp_wq, oppochg_usbtemp_condition() == true);
...
static int oppochg_set_icl(int current_ma)
{
	...
	if (oppochg_is_suspend_status() && oppochg_is_usb_present() && oppo_chg_is_normal_mode()) {
		i = i - 1;
		goto aicl_suspend;
	}
