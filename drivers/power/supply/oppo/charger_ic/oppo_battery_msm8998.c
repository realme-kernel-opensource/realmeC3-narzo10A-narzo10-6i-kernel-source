...
struct smb_dt_props {
	...
	int	usb_icl_ua;
#ifndef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2017/03/08, sjc Delete for OTG */
	int	otg_cl_ua;
#endif
	int	dc_icl_ua;
...
#ifdef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2017/07/17, sjc Modify for OTG interface fan */
...
#ifdef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2017/05/01, sjc Delete for charging */
...
#ifndef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2017/05/09, sjc Modiy for charging */
...
#define PSPNUM_OPPO_AC_PROPS        ARRAY_SIZE(oppo_ac_props);


static enum power_supply_property ac_props[ARRAY_SIZE(platform_ac_props) + ARRAY_SIZE(oppo_ac_props)] = {0};
static enum power_supply_property battery_props[ARRAY_SIZE(platform_batt_props) + ARRAY_SIZE(oppo_batt_props)] = {0};
static enum power_supply_property usb_props[ARRAY_SIZE(platform_usb_props) + ARRAY_SIZE(oppo_usb_props)] = {0};




void power_supply_property_merge(power_supply_property platform_props[], power_supply_property oppo_props[]) {
    int i;
    for(i = 0; i < ARRAY_SIZE(platform_props); i++) {
        ac_props[i] = platform_props[i];
    }
    for(i = 0; i < ARRAY_SIZE(oppo_props); i++) {
        ac_props[ARRAY_SIZE(platform_props) + i]= oppo_props[i];
    }
}
*/
...
/*
void oppo_chg_ac_psy_init(void)
{
		chip->ac_psd.name = "ac";
...
static int smb2_init_ac_psy(struct smb2 *chip)
{
    ...
    //power_supply_property_merge(platform_ac_props[],oppo_ac_props[]);
 //   g_oppo_chip->ac_cfg = ac_cfg;
//	g_oppo_chip->ac_psd = ac_psy_desc;
...
static int smb2_init_batt_psy(struct smb2 *chip)
{
   ...
   // g_oppo_chip->battery_psd= batt_psy_desc;
    //g_oppo_chip->battery_cfg = battery_cfg;
...
static int smbchg_set_fastchg_current_raw(int current_ma)
{
	...
	rc = vote(g_oppo_chip->pmic_spmi.smb2_chip->chg.fcc_votable, DEFAULT_VOTER,
...
static int smbchg_float_voltage_set(int vfloat_mv)
{
	...
	rc = vote(g_oppo_chip->pmic_spmi.smb2_chip->chg.fv_votable, DEFAULT_VOTER,
			true, vfloat_mv * 1000);
	if (rc < 0)
...
static int smbchg_term_current_set(int term_current)
{
	...
	rc = smblib_masked_write(&g_oppo_chip->pmic_spmi.smb2_chip->chg, TCCC_CHARGE_CURRENT_TERMINATION_CFG_REG,
...
static int smbchg_charging_enble(void)
{
	...
	rc = vote(g_oppo_chip->pmic_spmi.smb2_chip->chg.chg_disable_votable, DEFAULT_VOTER,
			false, 0);
	if (rc < 0)
...
static int smbchg_get_charge_enable(void)
{
	...
	rc = smblib_read(&g_oppo_chip->pmic_spmi.smb2_chip->chg, CHARGING_ENABLE_CMD_REG, &temp);
	if (rc < 0) {
...
static int smbchg_usb_suspend_enable(void)
{
	...
	rc = smblib_set_usb_suspend(&g_oppo_chip->pmic_spmi.smb2_chip->chg, true);
	if (rc < 0)
...
static int smbchg_usb_suspend_disable(void)
{
	...
	rc = smblib_set_usb_suspend(&g_oppo_chip->pmic_spmi.smb2_chip->chg, false);
	if (rc < 0)
...
static int smbchg_read_full(void)
{
	...
	rc = smblib_read(&g_oppo_chip->pmic_spmi.smb2_chip->chg, BATTERY_CHARGER_STATUS_1_REG, &stat);
	if (rc < 0) {
