...
//huangtongfeng@BSP.CHG.Basic, 2017/01/13, add for kpoc charging param.
	case Get_FakeOff_Param:
		...
		if(is_vooc_project()){
			fakeoff_out_data[4] = oppo_get_charger_chip_st();
		}else{
        ...
        union power_supply_propval *val)
{
	    ...
	    ret = oppo_usb_get_property(psy, psp, val);
        ...
        enum power_supply_property psp)
{
	    ...
	    ret = oppo_usb_property_is_writeable(psy, psp, val);
        ...
        const union power_supply_propval *val)
{
	    ...
	    ret = oppo_usb_set_property(psy, psp, val);
		
        return ret;
}


static int oppo_chg_usb_psy_init(struct oppo_chg_chip *chip)
{
        ...
        const union power_supply_propval *val)
{
        ...
        ret = oppo_battery_set_property(psy, prop, val);
...
{
        int ret = 0;
        struct oppo_chg_chip *chip = container_of(psy->desc, struct oppo_chg_chip, psp);
        ...
        switch (psp) {
            case POWER_SUPPLY_PROP_adjust_power:
                ...
                break;
            default:
                ret = oppo_battery_get_property(psy, psp, val);
                break;        
        }
...
static int oppo_chg_batt_psy_init(struct oppo_chg_chip *chip)
{
...
//kong
    rc = oppo_chg_usb_psy_init(chip);
    if (rc < 0) {
        pr_err("Couldn't initialize usb psy rc=%d\n", rc);
       return rc;
    } 
    
    rc = oppo_chg_ac_psy_init(chip);
    if (rc < 0) {
        pr_err("Couldn't initialize ac psy rc=%d\n", rc);
        return rc;
    }
//kong
    rc = oppo_chg_batt_psy_init(chip);
    if (rc < 0) {
...
struct  oppo_chg_chip  *g_charger_chip = NULL;
...
//kong
    ...
    ret = oppo_power_supply_init(chip);
    if (ret < 0) {
           pr_err("Couldn't initialize power supply psy rc=%d\n", rc);
           goto psy_reg_failed;
    } 
    
    chip->chg_ops = &(oppo_get_chg_ops());
    g_charger_chip = chip;
