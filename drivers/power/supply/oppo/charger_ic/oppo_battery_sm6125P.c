...
#include "oppo_battery_sm6125P.h"
...
/* Yichun.Chen  PSW.BSP.CHG  2018-05-10  use oppo battery*/
	rc = of_property_read_u32_array(pnode, "qcom,jeita-hard-thresholds",
        ...
        if (bat_thermal_1k == true) {
                rc = of_property_read_u32_array(pnode, "qcom,jeita-hard-thresholds_oppo",
                                        jeita_hard_thresholds, 2);
        } else {
                rc = of_property_read_u32_array(pnode, "qcom,jeita-hard-thresholds_oppo",
                                        jeita_hard_thresholds, 2);
...
#else
        if (bat_thermal_1k == true) {
                rc = of_property_read_u32_array(pnode, "qcom,jeita-soft-thresholds_oppo",
                                        chg->jeita_soft_thlds, 2);
        } else {
                rc = of_property_read_u32_array(pnode, "qcom,jeita-soft-thresholds_oppo",
                                        chg->jeita_soft_thlds, 2);
...
static void otg_enable_pmic_id_value (void)
{
	...
	if (rc < 0) {
		printk(KERN_ERR "[OPPO_CHG][%s]: Couldn't clear 0x1544[0] rc=%d\n", __func__, rc);\
	}

	rc = smblib_read(chg, TYPE_C_MODE_CFG_REG, &stat);
	if (rc < 0) {
		printk(KERN_ERR "[OPPO_CHG][%s]: Couldn't read 0x1544 rc=%d\n", __func__, rc);
	} else {
		printk(KERN_ERR "[OPPO_CHG][%s]: reg0x1544[0x%x], bit[2:0]=0(DRP)\n", __func__, stat);
	}
...
static void otg_disable_pmic_id_value (void)
{
	...
	if (rc < 0) {
		printk(KERN_ERR "[OPPO_CHG][%s]: Couldn't set 0x1544[2] rc=%d\n", __func__, rc);
	}
	...
	if (rc < 0) {
		...
		printk(KERN_ERR "[OPPO_CHG][%s]: reg0x1544[0x%x], bit[2:0]=4(UFP)\n", __func__, stat);
	}
...
/*LiYue@BSP.CHG.Basic, 2019/07/04, modefy for usb connector temp check*/
	...
	if (IS_ERR(oppo_chip->pmic_spmi.usb_temp_v_l_chan)) {
		...
		oppo_chip->pmic_spmi.usb_temp_v_l_chan = NULL;
	}

	oppo_chip->pmic_spmi.usb_temp_v_r_chan = iio_channel_get(oppo_chip->dev, "usb_temp_v_r");
	if (IS_ERR(oppo_chip->pmic_spmi.usb_temp_v_r_chan)) {
		dev_err(chg->dev, "Couldn't get usb_temp_v_r_chan...\n");
		oppo_chip->pmic_spmi.usb_temp_v_r_chan = NULL;
	}
#endif
