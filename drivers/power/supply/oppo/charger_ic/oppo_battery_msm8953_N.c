...
#ifdef CONFIG_OPPO_SOC_RESTORE_SUPPORT
...
static void qpnp_get_charger_type(struct oppo_chg_chip *chip, char **usb_type_name,
...
static int get_prop_batt_status(struct oppo_chg_chip *chip)
{
...
static int get_prop_charge_type(struct oppo_chg_chip *chip)
{
	int rc;
	u8 reg, chg_type;
...
static void get_property_from_typec(struct oppo_chg_chip *chip,
...
static void update_typec_status(struct oppo_chg_chip *chip)
{
...
static void use_pmi8994_tables(struct oppo_chg_chip *chip)
{
...
static void use_pmi8996_tables(struct oppo_chg_chip *chip)
{
...
static int smbchg_set_aicl_rerun_period_s(struct oppo_chg_chip *chip,
								int period_s)
{
	int i;
	u8 reg, mask;
...
static bool smbchg_is_usbin_active_pwr_src(struct oppo_chg_chip *chip)
{
	int rc;
	u8 reg;
...
static void smbchg_detect_parallel_charger(struct oppo_chg_chip *chip)
{
...
{
	int rc;
	struct oppo_chg_chip *chip = dev_get_drvdata(dev);
							...
							int current_ma)
{
	...
	rc = vote(chip->pmic_spmi.usb_icl_votable, PSY_ICL_VOTER,
			true, current_ma);
	if (rc < 0) {
		pr_err("Can't vote oppo_chg %d current limit rc=%d\n",
			current_ma, rc);
	}
...
static int smbchg_system_temp_level_set(struct oppo_chg_chip *chip,
								int lvl_sel)
{
	int rc = 0;
	int prev_therm_lvl;
...
static int smbchg_iterm_set(struct oppo_chg_chip *chip, int iterm_ma)
{
	int rc;
	u8 reg;
...
#if 0
static int smbchg_safety_timer_enable(struct oppo_chg_chip *chip, bool enable)
{
	int rc;
	u8 reg;
...
static int smbchg_switch_buck_frequency(struct oppo_chg_chip *chip,
				bool flash_active)
{
	int rc;
...
static int smbchg_otg_pulse_skip_disable(struct oppo_chg_chip *chip,
...
static int smbchg_set_prop_flash_active(struct oppo_chg_chip *chip,
				enum skip_reason reason, bool disable)
{
	int rc = 0;
...
static int smbchg_icl_loop_disable_check(struct oppo_chg_chip *chip)
{
...
static void check_battery_type(struct oppo_chg_chip *chip)
{
	union power_supply_propval prop = {0,};
	bool en;
		...
		enum led_brightness value)
{
	...
	struct oppo_chg_chip *chip = container_of(pmic_chip,
			struct oppo_chg_chip, pmic_spmi);
	u8 reg;
	int rc;
...
static void smbchg_chg_led_blink_set(struct oppo_chg_chip *chip,
...
static int smbchg_set_optimal_charging_mode(struct oppo_chg_chip *chip, int type)
{
	int rc;
...
static int smbchg_change_usb_supply_type(struct oppo_chg_chip *chip,
...
static int smbchg_restricted_charging(struct oppo_chg_chip *chip, bool enable)
{
...
static void increment_aicl_count(struct oppo_chg_chip *chip)
{
...
#if 0
static void update_typec_capability_status(struct oppo_chg_chip *chip,
...
static void update_typec_otg_status(struct oppo_chg_chip *chip, int mode,
					bool force)
{
...
static int smbchg_get_iusb(struct oppo_chg_chip *chip)
{
...
void oppo_set_usbid_active(struct oppo_chg_chip *chip)
{
...
void oppo_set_usbid_sleep(struct oppo_chg_chip *chip)
{
	gpio_direction_output(chip->normalchg_gpio.usbid_gpio,0);
...
void oppo_usbid_irq_init(struct oppo_chg_chip *chip)
{
	chip->normalchg_gpio.usbid_irq = gpio_to_irq(chip->normalchg_gpio.usbid_gpio);
}

void oppo_usbid_irq_register(struct oppo_chg_chip *chip)
{
	int retval = 0;
	union power_supply_propval ret = {0,}; 
	...
	if(chip->normalchg_gpio.usbid_gpio <= 0) {
		...
		if(oppo_usbid_check_is_gpio(chip) == true) {
			...
			if(rc){
				...
				oppo_usbid_gpio_init(chip);
				oppo_usbid_irq_init(chip);
			}
		}
	}
...
static void rerun_hvdcp_det_if_necessary(struct oppo_chg_chip *chip)
{
	enum power_supply_type usb_supply_type;
	char *usb_type_name;
	int rc;
...
#if 0
static int qpnp_charger_type_get(struct oppo_chg_chip *chip) 
{ 
    union power_supply_propval ret = {0,}; 
...
void otg_enable_id_value (void) 
{
	...
	if(oppo_usbid_check_is_gpio(the_chip) == true) {
			oppo_set_usbid_active(the_chip);
...
void otg_disable_id_value (void)
{
	...
	if(oppo_usbid_check_is_gpio(the_chip) == true) {
		//oppo_usbid_eint_unregister(the_chip);
		oppo_set_usbid_sleep(the_chip);
		usbid_change_handler(0, the_chip);
