...
static bool oppo_ccdetect_check_is_gpio(struct oppo_chg_chip *chip);
static int oppo_ccdetect_gpio_init(struct oppo_chg_chip *chip);
static void oppo_ccdetect_irq_init(struct oppo_chg_chip *chip);
static void oppo_ccdetect_disable(void);
...
#ifdef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2017/07/17, sjc Modify for OTG current limit (V3.1) */
...
#ifndef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2018/01/13, sjc Modify for charging */
...
#ifdef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2018/03/02, sjc Add for usbid */
...
#if 0
static void oppo_set_ccdetect_active(void)
{
	struct smb_charger *chg = NULL;
	struct oppo_chg_chip *chip = g_oppo_chip;
...
static void oppo_set_ccdetect_sleep(void)
{
	struct smb_charger *chg = NULL;
	struct oppo_chg_chip *chip = g_oppo_chip;
...
static int oppo_usbtemp_monitor_main(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (level == 0 || usbtemp_volt <= USB_50C_VOLT) {
			...
			wait_event_interruptible(oppo_usbtemp_wq, gpio_get_value(chg->ccdetect_gpio) == 0);
		}
...
static void dump_regs(void)
{
	...
	for (j = 0; j < 8; j++) {
		for (i = 0; i < 16; i++) {
			...
			printk(KERN_ERR "[OPPO_CHG]SMBCHG_REG: 0x%04x # [ 0x%02x / 0x%02x / 0x%02x / 0x%02x ], "
...
#ifdef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2017/01/22, sjc Add for charging*/
	if (of_find_property(oppo_chip->dev->of_node, "qcom,pm8998chg-vadc", NULL)) {
		oppo_chip->pmic_spmi.pm8998_vadc_dev = qpnp_get_vadc(oppo_chip->dev, "pm8998chg");
		if (IS_ERR(oppo_chip->pmic_spmi.pm8998_vadc_dev)) {
			rc = PTR_ERR(oppo_chip->pmic_spmi.pm8998_vadc_dev);
			oppo_chip->pmic_spmi.pm8998_vadc_dev = NULL;
			if (rc != -EPROBE_DEFER)
...
#ifdef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2018/05/25, sjc Add for usbtemp */
	if (of_find_property(oppo_chip->dev->of_node, "qcom,pm8998usbtemp-vadc", NULL)) {
		oppo_chip->pmic_spmi.pm8998_usbtemp_vadc_dev = qpnp_get_vadc(oppo_chip->dev, "pm8998usbtemp");
		if (IS_ERR(oppo_chip->pmic_spmi.pm8998_usbtemp_vadc_dev)) {
			rc = PTR_ERR(oppo_chip->pmic_spmi.pm8998_usbtemp_vadc_dev);
			oppo_chip->pmic_spmi.pm8998_usbtemp_vadc_dev = NULL;
			if (rc != -EPROBE_DEFER)
